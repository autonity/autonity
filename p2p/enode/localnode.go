...
func NewLocalNode(db *DB, key *ecdsa.PrivateKey, log log.Logger) *LocalNode {
	ln := &LocalNode{
...
func (ln *LocalNode) sign() {
	...
	ln.cur.Store(n)
	ln.log.Info("New local node record", "seq", ln.seq, "id", n.ID(), "ip", n.IPAddr(), "udp", n.UDP(), "tcp", n.TCP())
}
