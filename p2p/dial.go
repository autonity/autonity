	...
	for {
		...
		case node := <-nodesCh:
			if err := d.checkDial(node); err != nil {
				d.log.Trace("Discarding dial candidate", "id", node.ID(), "ip", node.IPAddr(), "reason", err, "server", d.net.String())
			} else {
		...
		case node := <-d.addStaticCh:
			...
			d.log.Trace("Adding static node", "id", id, "ip", node.IPAddr(), "added", !exists, "server", d.net.String())
			if exists {
				continue loop
			}
...
func (t *dialTask) resolve(d *dialScheduler) bool {
	...
	if resolved == nil {
		...
		d.log.Debug("Resolving node failed", "id", node.ID(), "newdelay", t.resolveDelay, "server", d.net.String())
		return false
	}
	// The node was found.
	t.resolveDelay = initialResolveDelay
...
func (t *dialTask) dial(d *dialScheduler, dest *enode.Node) error {
	...
	if err != nil {
		...
		// raise verbosity to warn if a dial
		//fails in the ACN network
		if d.net == Consensus {
			customLogFn = d.log.Warn
		} else {
			customLogFn = d.log.Trace
		}
		customLogFn("Dial error", "id", t.dest().ID(), "addr", t.dest().String(), "conn", t.flags, "err", cleanupDialErr(err), "server", d.net.String())
