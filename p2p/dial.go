	...
	for {
		...
		case node := <-nodesCh:
			if err := d.checkDial(node); err != nil {
				d.log.Trace("Discarding dial candidate", "id", node.ID(), "ip", node.IP(), "reason", err, "server", d.net.String())
			} else {
		...
		case node := <-d.addStaticCh:
			...
			d.log.Trace("Adding static node", "id", id, "ip", node.IP(), "added", !exists, "server", d.net.String())
			if exists {
				continue loop
			}
		...
		case node := <-d.remStaticCh:
			id := node.ID()
			task := d.static[id]
			d.log.Trace("Removing static node", "id", id, "ok", task != nil, "server", d.net.String())
			if task != nil {
				delete(d.static, id)
...
func (d *dialScheduler) logStats() {
	...
	if d.dialPeers < dialStatsPeerLimit && d.dialPeers < d.maxDialPeers {
		d.log.Info("Looking for peers", "peercount", len(d.peers), "tried", d.doneSinceLastLog, "static", len(d.static), "server", d.net.String())
	}
...
func (d *dialScheduler) startDial(task *dialTask) {
	...
	if d.net == Consensus {
		...
		d.history.add(hkey, d.clock.Now().Add(dialHistoryExpiration+randomDelay))
	}
...
func (t *dialTask) resolve(d *dialScheduler) bool {
	...
	if resolved == nil {
		...
		d.log.Debug("Resolving node failed", "id", t.dest.ID(), "newdelay", t.resolveDelay, "server", d.net.String())
		return false
	}
	// The node was found.
	t.resolveDelay = initialResolveDelay
	...
	t.dest = resolved
	t.lock.Unlock()
	d.log.Debug("Resolved node", "id", t.dest.ID(), "addr", &net.TCPAddr{IP: t.dest.IP(), Port: t.dest.TCP()}, "server", d.net.String())
	return true
}
...
func (t *dialTask) dial(d *dialScheduler, dest *enode.Node) error {
	...
	if err != nil {
		...
		customLogFn("Dial error", "id", t.dest.ID(), "addr", nodeAddr(t.dest), "conn", t.flags, "err", cleanupDialErr(err), "server", d.net.String())
