...
The dashboard is a data visualizer integrated into autonity, intended to collect and visualize useful information of an Ethereum node. It consists of two parts:
...
Normally the dashboard assets are bundled into Autonity via `go-bindata` to avoid external dependencies. Rebuilding Autonity after each UI modification however is not feasible from a developer perspective. Instead, we can run `yarn dev` to watch for file system changes and refresh the browser automatically.

```
$ autonity --dashboard --vmodule=dashboard=5
$ (cd dashboard/assets && yarn dev)
```

To bundle up the final UI into Autonity, run `go generate`:
