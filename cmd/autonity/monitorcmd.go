...
var (
	...
	monitorCommand = cli.Command{
		...
		Description: `
The Autonity monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) error {
	...
	if client, err = dialRPC(endpoint); err != nil {
		utils.Fatalf("Unable to attach to autonity node: %v", err)
	}
	defer client.Close()
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			utils.Fatalf("No metrics collected by autonity (--%s).\n", utils.MetricsEnabledFlag.Name)
		}
	}
...
// retrieveMetrics contacts the attached autonity node and retrieves the entire set
