...
func TestAccountListEmpty(t *testing.T) {
	t.Parallel()
	geth := runAutonity(t, "account", "list")
	geth.ExpectExit()
}
...
`
	...
	{
		geth := runAutonity(t, "account", "list", "--datadir", datadir)
		geth.Expect(want)
		geth.ExpectExit()
	}
	{
		geth := runAutonity(t, "--datadir", datadir, "account", "list")
		geth.Expect(want)
		geth.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	t.Parallel()
	geth := runAutonity(t, "account", "new", "--lightkdf")
...
func TestAccountHelp(t *testing.T) {
	t.Parallel()
	geth := runAutonity(t, "account", "-h")
	geth.WaitExit()
	if have, want := geth.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	geth = runAutonity(t, "account", "import", "-h")
	geth.WaitExit()
	if have, want := geth.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	geth := runAutonity(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer geth.ExpectExit()
...
func TestAccountUpdate(t *testing.T) {
	t.Parallel()
	datadir := tmpDatadirWithKeystore(t)
	geth := runAutonity(t, "account", "update",
