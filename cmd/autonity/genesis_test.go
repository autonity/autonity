...
// Test that if there is chain data with genesis block presented, start node with different genesis file, the node should
// not perform an genesis init, and the start up should be interrupted.
func TestStartNodeWithMismatchGenesis(t *testing.T) {
	for i, tt := range customGenesisTests {
		coinbase := "0x8605cdbbdb6d264aa742e77020dcbc58fcdce182"
		var ipc string
		if runtime.GOOS == "windows" {
		...
		// Then start node again with mismatch genesis, node should not init a new genesis, and the start up should be interrupted.
		jsonV2 := filepath.Join(datadir, "genesisV2.json")
		if err := ioutil.WriteFile(jsonV2, []byte(tt.genesisV2), 0600); err != nil {
			t.Fatalf("test %d: failed to write genesis file: %v", i, err)
		}
		autonity = runAutonity(t,
			"--port", "0", "--maxpeers", "0", "--nodiscover", "--nat", "none",
