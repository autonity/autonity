...
func thresholdSigning() {
	...
	_, secretShares, publicKey, err := DKG(n, t)
	if err != nil {
		panic(err)
	}
	...
	// TODO: there might be a off-by-one mistake somewhere as with the DKG shares I now need t+1 to sign correctly
	//       instead with shamir t was enough
	//       maybe this is because the polys are of order t?
	participatingShares := t + 1
...
func thresholdDecryption() {
	...
	secretScalar, secretShares, publicKey, err := DKG(n, t)
	if err != nil {
		panic(err)
	}
	...
	//       instead with shamir t was enough
	//       maybe this is because the polys are of order t?
	decryptionShares := make([]*blstbind.P1, 0, t+1)
	for i := 0; i <= t+1; i++ {
...
// ================================================================
// GJKR99 DKG implementation
// paper: Secure distributed key generation for discrete-log based cryptosystems by Gennaro et al.
// ================================================================

// --------------------------------------------------
// Global generators:
//
//	G: existing BLS12-381 G1 generator
//	H: independent generator via hash-to-curve (Option B1)
//
// --------------------------------------------------
var (
	G1Generator *blstbind.P1
	H1Generator *blstbind.P1
)

func init() {
	// G1 generator already available
	G1Generator = copyPoint(blstbind.P1Generator())

	// TODO: a simplification is applied here.
	//       H can be generated by all participants using a pedersen-VSS.
	//       see section 4.2 of the paper

	// Create H via hash-to-curve
	seed := make([]byte, 32)
	_, err := rand.Read(seed)
	if err != nil {
		panic("cannot generate seed for H")
	}

	H1Generator = blstbind.HashToG1(seed, generalDST)
}

// --------------------------------------------------
// Pedersen VSS Commitments
// C_{ik} = g^{a_{ik}}  (from f_i)
// D_{ik} = h^{b_{ik}}  (from f'_i)
// --------------------------------------------------

type PedersenCommitments struct {
	C []*blstbind.P1 // len t+1
	D []*blstbind.P1 // len t+1
}

// Create Pedersen commitments from f and f'
func commitPolynomials(f, fprime []*big.Int) *PedersenCommitments {
	t := len(f) - 1
	C := make([]*blstbind.P1, t+1)
	D := make([]*blstbind.P1, t+1)

	for k := 0; k <= t; k++ {
		C[k] = copyPoint(G1Generator)
		C[k].MultAssign(bigToScalar(f[k])) // g^{a_k}
		D[k] = copyPoint(H1Generator)
		D[k].MultAssign(bigToScalar(fprime[k])) // h^{b_k}
	}

	return &PedersenCommitments{C: C, D: D}
}

// --------------------------------------------------
// Shares sent from dealer i to receiver j:
//    sij  = f_i(j)
//    s'ij = f'_i(j)
// --------------------------------------------------

type DealerShares struct {
	S  []*big.Int // f_i(j) for j=1..n
	Sp []*big.Int // f'_i(j) for j=1..n
}

// Holds all data for one dealer Pi
type Dealer struct {
	Index int
	...
	Commit *PedersenCommitments
	Shares *DealerShares

	// Record of complaints received
	Complaints []int // list of complaining players indices
}

// --------------------------------------------------
// Complaint object: Pj complains about Pi
// --------------------------------------------------
...
// --------------------------------------------------
// Final structure for QUAL
// --------------------------------------------------
type QUALSet struct {
	Members []int
}

// Utility: return true if dealer is in QUAL
func (q *QUALSet) Contains(i int) bool {
	for _, x := range q.Members {
		if x == i {
			return true
		}
	}
...
// DealerStep1Result holds all information needed after Step 1.
type DealerStep1Result struct {
	Dealers    []*Dealer
	Complaints []Complaint
	QUAL       *QUALSet
}

// DistributedKeyGeneration_Part1 executes Step 1a–1d of GJKR99.
func distributedKeyGeneration_Part1(n, t int) (*DealerStep1Result, error) {
	// ------------------------------------------------------------
	// Step 1a: each player i acts as a dealer and creates f_i, f'_i
	// ------------------------------------------------------------
	dealers := make([]*Dealer, n+1) // we use 1-based indexing

	for i := 1; i <= n; i++ {
		// Create two random polys of deg t
		ai0, err := randomNonZeroScalar()
		if err != nil {
			return nil, err
		}
		bi0, err := randomNonZeroScalar()
		if err != nil {
			return nil, err
		}
		f, err := makePoly(t+1, ai0)
		if err != nil {
			return nil, err
		}
		fp, err := makePoly(t+1, bi0)
		if err != nil {
			return nil, err
		}

		// construct commitments
		commit := commitPolynomials(f, fp)

		// construct shares for j=1..n:
		S := make([]*big.Int, n+1)
		Sp := make([]*big.Int, n+1)
		for j := 1; j <= n; j++ {
			S[j] = evalPoly(f, big.NewInt(int64(j)))
			Sp[j] = evalPoly(fp, big.NewInt(int64(j)))
		}
	...
	// ------------------------------------------------------------
	// Step 1b: Each receiver P_j verifies shares from each dealer P_i
	// ------------------------------------------------------------
	complaints := []Complaint{}

	for i := 1; i <= n; i++ { // dealer
		for j := 1; j <= n; j++ { // receiver
			// Receiver j verifies dealer i's shares: s_ij, s'_ij
			sij := dealers[i].Shares.S[j]
			spij := dealers[i].Shares.Sp[j]

			// Left side: g^{s_ij} h^{s'_ij}
			L := copyPoint(G1Generator)
			L.MultAssign(bigToScalar(sij)) // g^{s_ij}

			temp := copyPoint(H1Generator)
			temp.MultAssign(bigToScalar(spij)) // h^{s'_ij}

			L.AddAssign(temp) // L = g^{s_ij} + h^{s'_ij} in group notation

			// Compute RHS = product_{k=0}^t (C_{ik} + D_{ik})^{j^k}
			R := copyPoint(dealers[i].Commit.C[0])
			R.AddAssign(copyPoint(dealers[i].Commit.D[0]))

			jBig := big.NewInt(int64(j))
			jPow := new(big.Int).Set(jBig) // j^1

			for k := 1; k <= t; k++ {
				// base = C_{ik} + D_{ik}
				base := copyPoint(dealers[i].Commit.C[k])
				base.AddAssign(copyPoint(dealers[i].Commit.D[k]))

				// compute base^(j^k)
				expPoint := copyPoint(base)
				expPoint.MultAssign(bigToScalar(jPow))

				// accumulate
				R.AddAssign(copyPoint(expPoint))
			...
			// Check Eq. (2)
			if !pointsEqual(L, R) {
				// Receiver j complains about dealer i
				complaints = append(complaints, Complaint{Dealer: i, From: j})
				dealers[i].Complaints = append(dealers[i].Complaints, j)
			}
		}
	}

	// ------------------------------------------------------------
	// Step 1c: Dealer i responds to complaints by broadcasting (s_ij, s'_ij)
	//          In this simulation we already have these values,
	//          so we only re-verify them for correctness.
	// ------------------------------------------------------------
	disqualified := make([]bool, n+1) // 1-based indexing

	for _, c := range complaints {
		i := c.Dealer
		j := c.From

		// Re-verify dealer i's shares for player j
		sij := dealers[i].Shares.S[j]
		spij := dealers[i].Shares.Sp[j]
		...
		if !pointsEqual(L, R) {
			// Dealer answered with faulty share → disqualified
			disqualified[i] = true
		}
	}

	// ------------------------------------------------------------
	// Step 1d: Disqualify any dealer with > t complaints
	// ------------------------------------------------------------
	for i := 1; i <= n; i++ {
		if len(dealers[i].Complaints) > t {
			disqualified[i] = true
		}
	}

	// ------------------------------------------------------------
	// Step 2: QUAL = all dealers that are not disqualified
	// ------------------------------------------------------------
	qualMembers := []int{}
	for i := 1; i <= n; i++ {
		if !disqualified[i] {
			qualMembers = append(qualMembers, i)
		}
	}

	QUAL := &QUALSet{Members: qualMembers}

	return &DealerStep1Result{
		Dealers:    dealers,
...
// ------------------------------------------------------------
// pointsEqual – compares two G1 points
// ------------------------------------------------------------
func pointsEqual(a, b *blstbind.P1) bool {
	aAff := a.ToAffine()
	bAff := b.ToAffine()
	return bytes.Equal(aAff.Serialize(), bAff.Serialize())
}

type DKGFinalResult struct {
	QUAL        *QUALSet
	Xj          []*big.Int                   // participant j final private share x_j
	Xpj         []*big.Int                   // participant j final private share x'_j
	PublicKey   *blstbind.P1                 // Y = g^x
	MasterX     *big.Int                     // x = Σ f_i(0)
	MasterXP    *big.Int                     // x' = Σ f'_i(0)
	Commitments map[int]*PedersenCommitments // commitments only for QUAL dealers
}

// distributedKeyGeneration_Part2 performs GJKR99 steps 3–5
func distributedKeyGeneration_Part2(n, t int, step1 *DealerStep1Result) (*DKGFinalResult, error) {

	QUAL := step1.QUAL
	dealers := step1.Dealers

	// ==================================================================
	// Step 4a and 4b — Feldman verification using commitments (already available)
	// ==================================================================
	//
	// Since the commitments were already properly checked during Step 1,
	// we only retain the commitments for QUAL dealers.
	//
	// In a real protocol, commitments would be redistributed and checked
	// for consistency here.

	commitMap := make(map[int]*PedersenCommitments)
	for _, i := range QUAL.Members {
		commitMap[i] = dealers[i].Commit
	}

	// assume no disqualifications for now
	if len(commitMap) != n {
		panic("something is wrong")
	}

	// ==================================================================
	// Step 4c + step 3 — Construct each participant's final shares
	//          x_j = Σ_{i in QUAL} f_i(j)
	//          x'_j = Σ_{i in QUAL} f'_i(j)
	// ==================================================================

	Xj := make([]*big.Int, n+1)  // 1-based
	Xpj := make([]*big.Int, n+1) // 1-based
	...
	for j := 1; j <= n; j++ {
		...
		for _, i := range QUAL.Members {
			fij := dealers[i].Shares.S[j]
			fpij := dealers[i].Shares.Sp[j]
	...
	// ==================================================================
	// Compute global secret key:
	//          Master secret: x = Σ f_i(0)
	// NOTE: this is not actually done in a real DKG protocol, no one ever accesses the master secret
	// however we compute it here for verification purposes
	// ==================================================================

	masterX := big.NewInt(0)
	masterXP := big.NewInt(0)

	for _, i := range QUAL.Members {
		masterX.Add(masterX, dealers[i].f[0])
		masterX.Mod(masterX, q)

		masterXP.Add(masterXP, dealers[i].fp[0])
		masterXP.Mod(masterXP, q)
	}

	// ==================================================================
	// Step 4c — Compute global public key using ONLY commitments:
	//
	//     Y = ∏ C_{i0}      (multiplicative notation)
	//       = Σ C_{i0}      (additive G1 notation, as in blst)
	//
	// ==================================================================

	var PublicKey *blstbind.P1 = nil

	for _, i := range QUAL.Members {
		// TODO: conflicts with 1 base indexing?
		CI0 := commitMap[i].C[0] // this is g^{a_{i0}}

		if PublicKey == nil {
			PublicKey = copyPoint(CI0)
		} else {
			PublicKey.AddAssign(CI0)
		}
	}

	// TODO: sanity check: verify public key matches the master secret
...
// DistributedKeyGeneration executes the full DKG (simulated, single process)
// n: total number of participants
// t: threshold
func DistributedKeyGeneration(n, t int) (*DKGFinalResult, error) {
	step1, err := distributedKeyGeneration_Part1(n, t)
	if err != nil {
		return nil, err
	}

	return distributedKeyGeneration_Part2(n, t, step1)
}

// TODO: the entire DKG process needs more cryptographic scrutiny to ensure correctness
func DKG(n int, t int) (*big.Int, []*big.Int, blst.PublicKey, error) {
	result, err := DistributedKeyGeneration(n, t)
	if err != nil {
		return nil, nil, nil, err
	}
	PrintDKGSummary(result)

	pubkey, err := blst.PublicKeyFromBytes(result.PublicKey.Compress())

	// NOTE: master secret key is returned also as the first secret share
	result.Xj[0] = new(big.Int).Set(result.MasterX)
	return result.MasterX, result.Xj, pubkey, nil
}

// ==================================================================
// Utility: Print DKG summary
// ==================================================================
func PrintDKGSummary(dkg *DKGFinalResult) {
	fmt.Println("QUAL members:", dkg.QUAL.Members)
	fmt.Println("Public key (G1 point):", dkg.PublicKey.ToAffine().Serialize())
	fmt.Println("Commitments per dealer:")
	for i, c := range dkg.Commitments {
		fmt.Printf("Dealer %d:\n", i)
		for k := 0; k < len(c.C); k++ {
			fmt.Printf("  C[%d] = %x\n", k, c.C[k].ToAffine().Serialize())
			fmt.Printf("  D[%d] = %x\n", k, c.D[k].ToAffine().Serialize())
		}
	}
}
