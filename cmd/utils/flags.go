...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	default:
		log.Info("Using Default Autonity Genesis Config")
		cfg.Genesis = core.DefaultAutonityGenesisBlock()
	}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "autonity.", map[string]string{
				"host": hosttag,
			})
		}
	}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	default:
		genesis = core.DefaultAutonityGenesisBlock()
	}
	return genesis
}
