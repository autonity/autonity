...
func (t *VerkleTrie) UpdateAccount(addr common.Address, acc *types.StateAccount, codeLen int) error {
	...
	if acc.Balance.ByteLen() > 16 {
		// todo:(youssef) this is a problem to make verkle trees compatible with autonity
		// I've been trying to look where this limitation is coming from but so far didn't find
