...
func TestFork(t *testing.T) {
	t.Skip("Fork tests require state history preservation which is not yet supported")
	testAddr := crypto.PubkeyToAddress(testKey.PublicKey)
	sim := simTestBackend(testAddr)
	defer sim.Close()
...
func TestCheckSimBackendGoroutineLeak(t *testing.T) {
	t.Skip("Miner goroutine leak is a known issue")
	...
	ignoreLdb := goleak.IgnoreTopFunction("github.com/syndtr/goleveldb/leveldb.(*DB).mpoolDrain")
	createAndCloseSimBackend()
