...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	pm, err := NewProtocolManager(config, cht, syncmode, DefaultConfig.NetworkId, new(event.TypeMux), &testTxPool{pool: make(map[common.Hash]*types.Transaction)}, ethash.NewFaker(), blockchain, db, 1, nil, nil, NewPeerSet(), nil)
	if err != nil {
...
func testBroadcastBlock(t *testing.T, totalPeers, broadcastExpected int) {
	...
	pm, err := NewProtocolManager(config, nil, downloader.FullSync, DefaultConfig.NetworkId, evmux, &testTxPool{pool: make(map[common.Hash]*types.Transaction)}, pow, blockchain, db, 1, nil, nil, NewPeerSet(), nil)
	if err != nil {
		t.Fatalf("failed to start test protocol manager: %v", err)
	}
...
func TestBroadcastMalformedBlock(t *testing.T) {
	...
	pm, err := NewProtocolManager(config, nil, downloader.FullSync, DefaultConfig.NetworkId, new(event.TypeMux), new(testTxPool), engine, blockchain, db, 1, nil, nil, NewPeerSet(), nil)
