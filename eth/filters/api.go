			...
			case txs := <-txs:
				...
				for _, tx := range txs {
					if fullTx != nil && *fullTx {
						rpcTx := ethapi.NewRPCPendingTransaction(tx, latest, chainConfig, api.sys.backend)
		...
		case PendingTransactionsSubscription:
			if f.fullTx {
				...
				for _, tx := range f.txs {
					txs = append(txs, ethapi.NewRPCPendingTransaction(tx, latest, chainConfig, api.sys.backend))
				}
				f.txs = nil
				return txs, nil
			} else {
