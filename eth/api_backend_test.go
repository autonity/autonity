...
func initBackend(t *testing.T, withLocal bool) *EthAPIBackend {
	...
	chain, _ := core.NewBlockChain(db, nil, gspec, engine, vm.Config{}, nil, core.FakeContractBackendProvider(t), log.Root())

	txconfig := legacypool.DefaultConfig
	txconfig.Journal = "" // Don't litter the disk with test journals

	legacyPool := legacypool.New(txconfig, chain)
	txpool, _ := txpool.New(txconfig.PriceLimit, chain, []txpool.SubPool{legacyPool})
