...
func New(stack *node.Node, config *Config) (*Ethereum, error) {
	...
	*txSender = eth.txPool.AddLocal
	...
	eth.accountability = accountability.NewFaultDetector(
		eth.blockchain,
		eth.address,
		evMux.Subscribe(events.MessageEvent{}),
...
func (s *Ethereum) newCommitteeWatcher() {
	...
	epochHead, currentHead := s.blockchain.LatestEpochHeadAndChainHead()

	if epochHead.CommitteeMember(s.address) != nil {
	...
	for {
		select {
		// todo: listen to new epoch event rather than chain head.
		case ev := <-chainHeadCh:
			epochHead, _ = s.blockchain.LatestEpochHeadAndChainHead()
