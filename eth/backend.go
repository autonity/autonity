...
func (s *Ethereum) genesisCountdown() {
	...
	for {
		...
		if days > 0 {
			if days != lastDays {
				lastDays = days
				s.log.Info(fmt.Sprintf("%d days remaining before genesis", days))
			}
		} else if hours == 1 || hours == 2 || hours == 6 || hours == 12 {
			if hours != lastHours {
				lastHours = hours
				s.log.Info(fmt.Sprintf("%d hours remaining before genesis", hours))
			}
		} else if (minutes == 1 || minutes == 5 || minutes == 15 || minutes == 30 || minutes == 45) && seconds == 0 {
			if minutes != lastMinute {
				lastMinute = minutes
				s.log.Info(fmt.Sprintf("%d minutes remaining before genesis", minutes))
			}
		} else if seconds < 10 || seconds == 30 || seconds == 45 {
			if seconds != lastSecond {
				lastSecond = seconds
				s.log.Info(fmt.Sprintf("%d seconds before genesis", seconds))
			}
		}
