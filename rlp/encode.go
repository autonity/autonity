...
// TODO: not sure if copy and return is necessary
func AppendToEncodedList(l []byte, b []byte) ([]byte, error) {
	if len(l) == 0 {
	...
	appendLength := uint64(len(b))
	newSize := expectedSize + appendLength
	...
	case isShortList && newSize <= 55:
		// remains short list
		list[0] = byte(0xC0 + expectedSize + appendLength)
	case isShortList && newSize > 55:
		// becomes long list, next case will update the size size
		list[0] = byte(0xF7)
		fallthrough
	case !isShortList:
		// update the size size
		...
		// if the new size size is bigger than the previous one, need to:
		// - update first byte
		// - add bytes after the first one
		if newSizeSize > sizeSize {
			list[0] = byte(0xF7 + newSizeSize)
			listExpanded := make([]byte, len(list)+int(newSizeSize-sizeSize))
			listExpanded[0] = byte(0xF7 + newSizeSize)
			copy(listExpanded[1+newSizeSize:], list[1:sizeSize])
			list = listExpanded
		}
