...
func (c *AutonityContract) callEpochByHeight(state vm.StateDB, header *types.Header, height *big.Int) (*types.EpochInfo, error) {
	...
	if _, err := c.AutonityContractCall(state, header, "getEpochByHeight", &[]any{&committeeMembers, &previousEpochBlock, &curEpochBlock, &nextEpochBlock, &delta}, height); err != nil {
		return nil, err
	}
	...
	epochInfo := &types.EpochInfo{
		Epoch:      types.Epoch{Committee: committee, PreviousEpochBlock: previousEpochBlock, NextEpochBlock: nextEpochBlock, Delta: delta},
...
func (c *AutonityContract) callGetEpochInfo(state vm.StateDB, header *types.Header) (*types.EpochInfo, error) {
	...
	if _, err := c.AutonityContractCall(state, header, "getEpochInfo", &[]any{&committeeMembers, &previousEpochBlock, &curEpochBlock, &nextEpochBlock, &delta}); err != nil {
		return nil, err
	}
	committee := &types.Committee{}
	committee.Members = committeeMembers
	...
	epoch := &types.EpochInfo{
		EpochBlock: curEpochBlock,
		Epoch:      types.Epoch{Committee: committee, NextEpochBlock: nextEpochBlock, PreviousEpochBlock: previousEpochBlock, Delta: delta},
	}
	return epoch, nil
}
