...
// it returns the committee, previousEpochBlock, curEpochBlock, nextEpochBlock and the current delta
func (c *AutonityContract) callGetEpochInfo(state vm.StateDB, header *types.Header) (*types.Committee, uint64, uint64, uint64, uint64, error) {
	var committeeMembers []types.CommitteeMember
	...
	return committee, previousEpochBlock.Uint64(), curEpochBlock.Uint64(), nextEpochBlock.Uint64(), delta.Uint64(), nil
}
