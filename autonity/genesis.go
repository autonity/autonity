...
func deployStakableVestingContract(config *params.ChainConfig, _ GenesisBonds, deploy genericDeployer, _ genericCaller) error {
	...
	if addr != params.StakableVestingContractAddress {
		return errBadDeploymentAddress
	}
	return nil
}
...
func createDefaultStakingVestingContracts(config *params.ChainConfig, _ GenesisBonds, _ genericDeployer, caller genericCaller) error {
	if _, err := caller(
		...
		"mint",
		params.StakableVestingContractAddress,
		config.StakableVestingConfig.TotalNominal,
	); err != nil {
	...
	callNewStakableContract := func(data params.StakableVestingData) error {
		_, err := caller(
			config.AutonityContractConfig.Operator,
			params.StakableVestingContractAddress,
			&generated.StakableVestingAbi,
	...
	for i, data := range config.StakableVestingConfig.StakableContracts {
...
func deployNonStakableVestingContract(config *params.ChainConfig, _ GenesisBonds, deploy genericDeployer, _ genericCaller) error {
	...
	if addr != params.NonStakableVestingContractAddress {
		return errBadDeploymentAddress
	}
	return nil
}
...
func createDefaultNonStakableVestingContracts(config *params.ChainConfig, _ GenesisBonds, _ genericDeployer, caller genericCaller) error {
	if _, err := caller(
		config.AutonityContractConfig.Operator,
		params.NonStakableVestingContractAddress,
		&generated.NonStakableVestingAbi,
		"setTotalNominal",
		config.NonStakableVestingConfig.TotalNominal,
	); err != nil {
	...
	if _, err := caller(
		...
		config.NonStakableVestingConfig.MaxAllowedDuration,
	); err != nil {
		return fmt.Errorf("error while seting max allowed duration in non-stakable vesting contract: %w", err)
	}

	createNonStakableSchedule := func(data params.NonStakableSchedule) error {
	...
	for _, schedule := range config.NonStakableVestingConfig.NonStakableSchedules {
		if err := createNonStakableSchedule(schedule); err != nil {
			return fmt.Errorf("error while creating new non-stakable schedule: %w", err)
		}
	}

	newNonStakableContract := func(data params.NonStakableVestingData) error {
	...
	for _, data := range config.NonStakableVestingConfig.NonStakableContracts {
		if err := newNonStakableContract(data); err != nil {
