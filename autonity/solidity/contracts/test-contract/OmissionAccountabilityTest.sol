...
// this test version is exactly identical to the production version
// except for the fact that it updates the inactivity score with the one
// specified when calling `setInactivityScore`.
// see function _computeInactivityScoresAndCollusionDegree for the modifications.
//
// this contract is meant to be deployed by using evm.Replace

contract OmissionAccountabilityTest is IOmissionAccountability, IConfigEvents, ReentrancyGuard {
    ...
    // test contract additional storage vars
    mapping(address => uint256) internal customInactivityScores;
    mapping(address => bool) internal hasCustomScore;
    ...
    */
    ...
    // note: will get reset after being applied once
    function setInactivityScore(address validator, uint256 score) external {
        customInactivityScores[validator] = score;
        hasCustomScore[validator] = true;
    }

    // convenience function to update the scores
    // care needs to be taken to call it at the right timing,
    // or it will mess up the state
    function updateScores() external {
        _computeInactivityScoresAndCollusionDegree();
    }
    ...
    function _computeInactivityScoresAndCollusionDegree() internal virtual returns (uint256) {
        ...
        for (uint256 i = 0; i < committee.length; i++) {
            ...
            */
            ...
            // test version addition: custom score handling
            if(hasCustomScore[_nodeAddress]){
                _aggregatedInactivityScore = customInactivityScores[_nodeAddress];
                delete customInactivityScores[_nodeAddress];
                delete hasCustomScore[_nodeAddress];
            }
    ...
    function _punishInactiveValidators(uint256 _collusionDegree) internal virtual {
        ...
        for (uint256 i = 0; i < committee.length; i++) {
            ...
            IAutonity.ValidatorState _state = autonity.getValidatorState(_nodeAddress);
            if (_state == IAutonity.ValidatorState.jailed || _state == IAutonity.ValidatorState.jailbound) {
                continue;
            }
            ...
            if (inactivityScores[_nodeAddress] <= config.inactivityThreshold) {
                ...
                if (probationPeriods[_nodeAddress] > 0) {
                    ...
                    (_slashingAmount, _jailReleaseBlock, _isJailbound) = autonity.slashAndJail(
                        ...
                        _slashingRate,
                        _jailingPeriod,
                        IAutonity.ValidatorState.jailedForInactivity,
                        IAutonity.ValidatorState.jailboundForInactivity
                    );
                    emit InactivitySlashingEvent(_nodeAddress, _slashingAmount, _jailReleaseBlock, _isJailbound);
                } else {
                    // if not, only jail
                    uint256 _jailReleaseBlock = autonity.jail(_nodeAddress, _jailingPeriod, IAutonity.ValidatorState.jailedForInactivity);
