 ...
 */
contract OracleTest is IOracle, IConfigEvents, ReentrancyGuard {
    ...
    */
    ...
    function setRewardPeriodAggregatedScore(uint256 score) external {
        require(score > 0, "score needs to be strictly positive");
        rewardPeriodAggregatedScore = score;
    }

    function getRewardPeriodAggregatedScore() external view returns (uint256) {
        return rewardPeriodAggregatedScore;
    }

    function setPerformance(address voter, uint256 score) external {
        require(score > 0, "score needs to be strictly positive");
        rewardPeriodPerformance[voter] = score;
        rewardReceivers.add(voter);
    }
