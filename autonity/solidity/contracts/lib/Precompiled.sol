...
library Precompiled {
    ...
    function updateLatency(uint256 _lastMatrixSlot, uint256 _reportsSlot, uint256 _matrixSlot, uint256 _index, uint8[] memory _latency) internal returns (uint256) {
        uint256[1] memory retVal;
        bytes memory input = abi.encodePacked(_lastMatrixSlot, _reportsSlot, _matrixSlot, _index, _latency);
