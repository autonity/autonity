...
library Precompiled {
    ...
    // height of the accountability event against to.
    // returns(msgSender, msgHash, result, ruleID, msghash)
    function verifyAccountabilityAccusation(address _to, bytes memory _proof, uint256 _range, uint256 _delta) internal view returns
    (bool _success, address _offender, uint256 _ruleId, uint256 _block, uint256 _msgHash) {

        bytes memory input = abi.encodePacked(_range, _delta, _proof);
        uint256[5] memory _returnData;
        assembly {
        //staticcall(gasLimit, to, inputOffset, inputSize, outputOffset, outputSize)
