...
struct Config {
    ...
    /// debt.
    uint256 minCollateralisationRatio;
...
/// @title ASM Stabilisation Contract
/// @notice A CDP-based stabilisation mechanism for the Auton.
/// @dev Intended to be deployed by the protocol at genesis. The operator is
/// expected to be the Autonity Contract. Note that all rates, ratios, prices,
/// and amounts are represented as fixed-point integers with `SCALE` decimal
/// places.
/* solhint-disable not-rely-on-time */
contract Stabilisation {
    /// Represents a Collateralised Debt Position (CDP)
    struct CDP {
        /// The timestamp of the last borrow or repayment.
        uint timestamp;
        /// The collateral deposited with the Stabilisation Contract.
        uint256 collateral;
    ...
    /// The Config object that stores Stabilisation Contract parameters.
    ...
    modifier onlyOperator() {
        if (msg.sender != _operator) revert Unauthorised();
        _;
    }
    ...
    modifier validRatios(
        ...
        if (liquidationRatio >= minCollateralisationRatio)
            revert InvalidParameter();
        _;
    }

    /// Create and deploy the ASM Stabilisation Contract.
    /// @param config_ Stabilisation configuration
    /// @param operator The operator is authorised to change parameters
    ...
    constructor(
        ...
        positiveMCR(config_.minCollateralisationRatio)
        validRatios(config_.liquidationRatio, config_.minCollateralisationRatio)
    {
        config = config_;
    ...
    */
    ...
    /// Stabilisation contract to spend Collateral Token on their behalf for
    ...
    function withdraw(uint256 amount) external nonZeroAmount(amount) {
        ...
        if (
            ...
            minimumCollateral(
                cdp.principal,
                price,
                config.minCollateralisationRatio
            )
    ...
    */
    ...
    /// Liquidate a CDP that is undercollateralised.
    ///
    ...
    */
    ...
    ///
    /// Must be less than the minimum collateralisation ratio.
    /// @dev Restricted to the operator.
    function setLiquidationRatio(
        uint256 ratio
    )
        external
        validRatios(ratio, config.minCollateralisationRatio)
        onlyOperator
    {
        config.liquidationRatio = ratio;
    }

    /// Set the minimum collateralisation ratio.
    ///
    /// Must be positive and greater than the liquidation ratio.
    /// @dev Restricted to the operator.
    function setMinCollateralisationRatio(
        uint256 ratio
    )
        external
        positiveMCR(ratio)
        ...
        onlyOperator
    {
        config.minCollateralisationRatio = ratio;
    }
    ...
    */
    ...
    /// @param mcr The minimum collateralisation ratio
    ...
    /// Determine if a debt position is undercollateralised.
