...
contract LiquidRewardManager {
    ...
    function _addValidator(uint256 _id, address _validator) private {
        ...
        if (address(rewardTracker[_validator].liquidStateContract) == address(0)) {
    ...
    function _liquidStateContract(address _validator) internal returns (ILiquidLogic) {
        RewardTracker storage _rewardTracker = rewardTracker[_validator];
        if (address(_rewardTracker.liquidStateContract) == address(0)) {
            _initiateValidator(_validator);
        }
        return _rewardTracker.liquidStateContract;
    }
