...
contract VestingManager is IStakeProxy, LiquidRewardManager {
    ...
    function unbond(uint256 _id, address _validator, uint256 _amount) virtual public onlyActive(_id) returns (uint256) {
        ...
        if (_lockedLiquidBalanceOf(_scheduleID, _validator) == 0) {
            if (scheduleUnbondingValidators[_scheduleID].length == 0) {
                unbondingSchedule.push(_scheduleID);
            }
            scheduleUnbondingValidators[_scheduleID].push(_validator);
        }
    ...
    //         if (_rejected) {
    //             _transferNTN(cancelRecipient[_scheduleID], _schedule.currentNTNAmount);
    //         }
    //         else {
    ...
    //         _updateScheduleTotalValue(_scheduleID, _validator, _request.amount, _liquid, _epochID());
    ...
    //         schedules[_scheduleID].totalValue -= _calculateLNTNValue(_ratio.liquidSupply, _ratio.delegatedStake, _unbondingRequst.amount);
    //     }
    ...
    //         return;
    //     }
    //     _item.currentNTNAmount += _amount;
    //     _item.totalValue += _amount;
    //     // TODO: try without removing the elements and compare gas usage
    // }
     ...
     */
    function _finalize(
        BondingApplied[] memory _appliedBonding,
        uint256[] memory _rejectedBonding,
        UnbondingReleased[] memory _releasedUnbonding
    ) internal {
