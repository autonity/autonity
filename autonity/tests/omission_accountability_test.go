...
func TestInactivityScore(t *testing.T) {
	...
	r.omissionAccountability.SetLastEpochBlock(fromAutonity, new(big.Int).SetUint64(uint64(omissionEpochPeriod)))
...
func TestOmissionPunishments(t *testing.T) {
	...
	r.autonity.UpdateValidatorAndTransferSlashedFunds(&runOptions{origin: r.accountability.address}, val1)
...
func TestProposerRewardDistribution(t *testing.T) {
	...
	t.Run("Rewards are correctly distributed among proposers", func(t *testing.T) {
		...
		for i, val := range r.committee.validators {
			...
			// effort counters should be zerod out
