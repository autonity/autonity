...
func absenteesLastHeight(r *Runner) []common.Address {
	...
	absentees := make([]common.Address, 0)
	for i := 0; i < csize; i++ {
		addr, _, err := r.OmissionAccountability.AbsenteesLastHeight(nil, new(big.Int).SetUint64(uint64(i)))
		if err != nil {
			break // out of boundary access will cause revert. We use that to indirectly determine the length of the array
		}
		absentees = append(absentees, addr)
	}
	return absentees
}
