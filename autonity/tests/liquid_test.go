...
func TestLogicOperation(t *testing.T) {
	...
	r.Run("liquid logic can be updated", func(r *Runner) {
		...
		liquidLogicFromAutonity, _, err := r.Autonity.GetLiquidLogicContract(nil, common.Address{})
...
func deployLiquid(
	...
	liquidLogic, _, err := r.Autonity.GetLiquidLogicContract(nil, common.Address{})
	require.NoError(r.T, err)
