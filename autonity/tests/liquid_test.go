...
func TestAccess(t *testing.T) {
	...
	r.Run("only autonity can lock", func(r *Runner) {
		_, err := liquidState.LockInPool(
			nil, validator, common.Address{}, common.Big1,
		)
		require.Error(r.T, err)
		require.Equal(r.T, "execution reverted: Call restricted to the Autonity Contract", err.Error())
	})
...
func TestFunctions(t *testing.T) {
	var (
		...
		delegatorA    common.Address
		delegatorB    common.Address
		delegatorC    common.Address
		liquidState   *ILiquid
	...
	RunWithSetup("reward single validator", setup, func(r *Runner) {
		// Initial state
		...
		commissionRate, _, err := r.Autonity.GetValidatorCommissionRate(nil, validator)
		require.NoError(r.T, err)
		atnDistributed, _, err := liquidState.CallRedistribute(r, FromSender(r.Autonity.address, liquidReward), common.Big0, commissionRate)
		require.NoError(r.T, err)
	...
	RunWithSetup("reward multiple validators", setup, func(r *Runner) {
	...
	RunWithSetup("transfer LNEW", setup, func(r *Runner) {
		// delegatorA bonds 8000 NEW
		// delegatorB bonds 2000 NEW
		// 20 AUT reward
		delegatorAMint := new(big.Int).Mul(big.NewInt(8000), params.DecimalFactor)
	...
	RunWithSetup("burn LNEW", setup, func(r *Runner) {
	...
	RunWithSetup("claiming rewards", setup, func(r *Runner) {
	...
	RunWithSetup("accumulating rewards", setup, func(r *Runner) {
	...
	RunWithSetup("commission", setup, func(r *Runner) {
		// use 50% commission for simplcity
		commissionRate := big.NewInt(50 * 100)
		...
		liquidReward := new(big.Int).Mul(big.NewInt(40), params.DecimalFactor)
		redistributeLiquidReward(r, newLiquidState, liquidReward, commissionRate)
	...
	RunWithSetup("allowances", setup, func(r *Runner) {
		// delegatorA bonds 10000 NEW
		delegatorAMint := new(big.Int).Mul(big.NewInt(10000), params.DecimalFactor)
		r.NoError(
			liquidState.Mint(FromAutonity, delegatorA, delegatorAMint),
	...
	RunWithSetup("locking", setup, func(r *Runner) {
		...
		checkLockedLiquidBalance(r, liquidState, delegatorA, balance, common.Big0)

		// lock more than balance
		_, err := liquidState.LockInPool(
			FromAutonity, delegatorA, common.Address{}, new(big.Int).Add(balance, common.Big1),
		)
		require.Error(r.T, err)
		...
		r.NoError(
			r.Autonity.Unbond(
				FromSender(delegatorA, nil),
				validator,
				balanceToLock,
			),
		)
		checkLockedLiquidBalance(r, liquidState, delegatorA, balance, balanceToLock)
		...
		r.NoError(
			liquidState.Transfer(
				FromSender(delegatorA, nil), delegatorB, maxTransferable,
			),
		)
		checkLockedLiquidBalance(r, liquidState, delegatorA, new(big.Int).Sub(balance, maxTransferable), balanceToLock)
		checkLockedLiquidBalance(r, liquidState, delegatorB, maxTransferable, common.Big0)
	})

	RunWithSetup("check name and symbol", setup, func(r *Runner) {
		liquidState = deployLiquid(r, validator, treasury)
...
func checkLockedLiquidBalance(r *Runner, liquidState *ILiquid, user common.Address, totalExpectedBalance, expecedLockedBalance *big.Int) {
	lockedBalance, _, err := liquidState.LockedBalanceOf(nil, user)
	require.NoError(r.T, err)
	require.True(r.T, lockedBalance.Cmp(expecedLockedBalance) == 0)

	unlockedBalance, _, err := liquidState.UnlockedBalanceOf(nil, user)
	require.NoError(r.T, err)
	require.True(r.T, unlockedBalance.Cmp(new(big.Int).Sub(totalExpectedBalance, expecedLockedBalance)) == 0)

	balance, _, err := liquidState.BalanceOf(nil, user)
	require.NoError(r.T, err)
	require.True(r.T, balance.Cmp(totalExpectedBalance) == 0)
}
...
func redistributeLiquidReward(r *Runner, liquidState *ILiquid, reward *big.Int, rate ...*big.Int) {
	validator, _, err := liquidState.GetValidator(nil)
	require.NoError(r.T, err)
	var commissionRate *big.Int
	if len(rate) == 0 {
		commissionRate, _, err = r.Autonity.GetValidatorCommissionRate(nil, validator)
		require.NoError(r.T, err)
	} else {
...
func updateCommissionRate(r *Runner, validator common.Address, commissionRatePercent int64) {
	...
	currentCommission, _, err := r.Autonity.GetValidatorCommissionRate(nil, validator)
	require.NoError(r.T, err)
	for currentCommission.Int64() != commissionRatePercent*100 {
		r.WaitNextEpoch()
		validatorInfo, _, err = r.Autonity.GetValidator(nil, validator)
		require.NoError(r.T, err)
		currentCommission, _, err = r.Autonity.GetValidatorCommissionRate(nil, validator)
