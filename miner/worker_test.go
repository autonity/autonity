...
func testGenerateBlockAndImport(t *testing.T, isTendermint bool) {
	...
	if isTendermint {
		...
		engine = tendermintBackend.New(testUserKey, testConsensusKey, &vm.Config{}, nil, evMux, msgStore, log.Root(), false, nil)
	} else {
...
func TestEmptyWorkTendermint(t *testing.T) {
	...
	testEmptyWork(t, tendermintChainConfig,
		tendermintBackend.New(testUserKey, testConsensusKey, new(vm.Config), nil, evMux, msgStore, log.Root(), false, nil),
		true)
}
