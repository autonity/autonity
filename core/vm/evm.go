...
func (evm *EVM) Call(caller common.Address, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if isPrecompile {
		ret, gas, err = RunPrecompiledContract(p, input, gas, evm.Context.BlockNumber.Uint64(), evm, caller, nil)
	} else {
...
func (evm *EVM) CallCode(caller common.Address, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if p, isPrecompile := evm.precompile(addr); isPrecompile {
		ret, gas, err = RunPrecompiledContract(p, input, gas, evm.Context.BlockNumber.Uint64(), evm, caller, evm.Config.Tracer)
	} else {
		// Initialise a new contract and set the code that is to be used by the EVM.
		// The contract is a scoped environment for this execution context only.
...
// Replace change the contract code section with a new one provided as parameter.
...
// easier to parse.
func (evm *EVM) Replace(caller common.Address, code []byte, address common.Address) (ret []byte, createAddress common.Address, leftOverGas uint64, err error) {
	gas := uint64(math.MaxUint64)
	if evm.Config.Tracer != nil {
		evm.captureBegin(evm.depth, CREATE, caller, address, code, gas, common.Big0)
		defer func(startGas uint64) {
			evm.captureEnd(evm.depth, startGas, leftOverGas, ret, err)
		}(gas)
	}
	...
	// Initialise a new contract and set the code that is to be used by the EVM.
	// The contract is a scoped environment for this execution context only.
	contract := NewContract(caller, address, uint256.NewInt(0), gas, evm.jumpDests)
...
func (evm *EVM) CreateWithAddress(caller common.Address, code []byte, gas uint64, value *uint256.Int, addr common.Address) (ret []byte, contractAddr common.Address, leftOverGas uint64, err error) {
