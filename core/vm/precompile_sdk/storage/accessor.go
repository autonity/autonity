...
type ValueAccessor interface {
	...
	WriteAt(slot common.Hash, offset int, value any, st *Storage) common.Hash
}
...
func init() {
	...
	registry[reflect.TypeOf(Address{})] = AddressAccessor{}
...
func (u Uint256Accessor) ReadAt(slot common.Hash, _ int, st *Storage) (any, error) {
	var u256 Uint256
	u256.SetBytes(slot[:])
	return u256, nil
}

func (u Uint256Accessor) WriteAt(slot common.Hash, _ int, value any, st *Storage) common.Hash {
	var u256 = value.(Uint256)
	u256.WriteToSlice(slot[:])
	return slot
}
...
func (u AddressAccessor) ReadAt(slot common.Hash, offset int, st *Storage) (any, error) {
	return NewAddressFromBytes(slot[offset : offset+20]), nil
}

func (a AddressAccessor) WriteAt(slot common.Hash, offset int, value any, st *Storage) common.Hash {
	var addr = value.(Address)
	copy(slot[offset:offset+20], zeroAddressBytes)
	copy(slot[offset:offset+20], addr.ToCommonAddress().Bytes())
	return slot
}
...
func (u uintAccessor) ReadAt(slot common.Hash, offset int, st *Storage) (any, error) {
	...
	fieldBytes := slot[offset : offset+u.size]
	switch u.size {
	case 1:
...
func (u uintAccessor) WriteAt(slot common.Hash, offset int, value any, st *Storage) common.Hash {
	if offset+u.size > 32 {
		panic(fmt.Errorf("offset and size exceed slot boundary"))
	}
	copy(slot[offset:offset+u.size], zeroHashBytes[:u.size])
	switch val := value.(type) {
	case uint8:
		slot[offset] = val
	case uint16:
		binary.BigEndian.PutUint16(slot[offset:offset+u.size], val)
	case uint32:
		binary.BigEndian.PutUint32(slot[offset:offset+u.size], val)
	case uint64:
		binary.BigEndian.PutUint64(slot[offset:offset+u.size], val)
	default:
...
func (i intAccessor) WriteAt(slot common.Hash, offset int, value any, st *Storage) common.Hash {
...
func (b boolAccessor) ReadAt(slot common.Hash, offset int, st *Storage) (any, error) {
	return slot[offset] == 1, nil
}

func (b boolAccessor) WriteAt(slot common.Hash, offset int, value any, st *Storage) common.Hash {
	slot[offset] = 0
	if value.(bool) {
		slot[offset] = 1
	}
...
func (b ByteAccessor) WriteAt(headSlot common.Hash, _ int, value any, st *Storage) common.Hash {
	if st == nil {
	...
	st.stateDB.SetState(st.address, headSlot, head) // Write length
	if length == 0 {
		return head
	}
	if length > 1<<20 {
