...
func (st *insertStats) report(chain []*types.Block, index int, snapDiffItems, snapBufItems, trieDiffNodes, triebufNodes common.StorageSize, setHead bool) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		if setHead {
			st.log.Info("Imported new chain segment", context...)
		} else {
			st.log.Info("Imported new potential chain segment", context...)
		}
		// Bump the stats reported to the next section
		*st = insertStats{startTime: now, lastIndex: index + 1, log: st.log}
	}
}
