...
func TestHeaderVerificationNew(t *testing.T) {
...
func testHeaderVerification(t *testing.T, scheme string) {
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), DefaultCacheConfigWithScheme(scheme), gspec, ethash.NewFaker(), vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	defer chain.Stop()
	if err != nil {
		t.Fatal(err)
	}
...
func TestCalcGasLimitNew(t *testing.T) {
	for i, tc := range []struct {
		...
		// Increase
		if have, want := CalcGasLimit(tc.pGasLimit, 2*tc.pGasLimit, params.DefaultGasLimitBoundDivisor), tc.max; have != want {
			t.Errorf("test %d: have %d want <%d", i, have, want)
		}
		// Decrease
		if have, want := CalcGasLimit(tc.pGasLimit, 0, params.DefaultGasLimitBoundDivisor), tc.min; have != want {
			t.Errorf("test %d: have %d want >%d", i, have, want)
		}
		// Small decrease
		if have, want := CalcGasLimit(tc.pGasLimit, tc.pGasLimit-1, params.DefaultGasLimitBoundDivisor), tc.pGasLimit-1; have != want {
			t.Errorf("test %d: have %d want %d", i, have, want)
		}
		// Small increase
		if have, want := CalcGasLimit(tc.pGasLimit, tc.pGasLimit+1, params.DefaultGasLimitBoundDivisor), tc.pGasLimit+1; have != want {
		...
		// No change
		if have, want := CalcGasLimit(tc.pGasLimit, tc.pGasLimit, params.DefaultGasLimitBoundDivisor), tc.pGasLimit; have != want {
			t.Errorf("test %d: have %d want %d", i, have, want)
		}
	}
}
