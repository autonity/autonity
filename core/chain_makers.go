...
func (b *BlockGen) AddUncle(h *types.Header) {
	...
	if b.cm.config.IsLondon(h.Number) {
		...
		h.BaseFee = misc.CalcBaseFee(b.cm.config, parent, eip1559Params)
...
func GenerateChain(config *params.ChainConfig, parent *types.Block, engine consensus.Engine, db ethdb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	...
	genblock := func(i int, parent *types.Block, triedb *triedb.Database, statedb *state.StateDB) (*types.Block, types.Receipts) {
		...
		b.header = cm.makeHeader(chainreader, parent, statedb, b.engine)
...
func (cm *chainMaker) makeHeader(chain consensus.ChainReader, parent *types.Block, state *state.StateDB, engine consensus.Engine) *types.Header {
	time := parent.Time() + 10 // block time is fixed at 10 seconds
	...
	if cm.config.IsLondon(header.Number) {
		eip1559Params, _ := chain.Eip1559ParamsByHeight(header.Number.Uint64())
		header.BaseFee = misc.CalcBaseFee(cm.config, parentHeader, eip1559Params)
		if !chain.Config().IsLondon(parent.Number()) {
			parentGasLimit := parent.GasLimit() * params.DefaultElasticityMultiplier
			header.GasLimit = CalcGasLimit(parentGasLimit, parentGasLimit, params.DefaultGasLimitBoundDivisor)
		}
