...
func TestDeriveFields(t *testing.T) {
	...
	// Autonity: Duplicated last receipt to account for the AC receipt
	receipts = append(receipts, receipts[len(receipts)-1])
	...
	r1, err := json.MarshalIndent(receipts[:len(receipts)-1], "", "  ")
	if err != nil {
		t.Fatal("error marshaling input receipts:", err)
	}

	r2, err := json.MarshalIndent(derivedReceipts[:len(derivedReceipts)-1], "", "  ")
	if err != nil {
