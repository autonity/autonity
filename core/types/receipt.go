...
func (rs Receipts) DeriveFields(config *params.ChainConfig, hash common.Hash, number uint64, baseFee *big.Int, txs []*Transaction) error {
	...
	logIndex := uint(0)
	// Autonity blocks have an extra finalization receipt (len(rs) == len(txs) + 1)
	// Regular blocks have matching counts (len(rs) == len(txs))
	hasAutonityReceipt := len(rs) == len(txs)+1
	if !hasAutonityReceipt && len(rs) != len(txs) {
		return errors.New("transaction and receipt count mismatch")
	}
	for i := 0; i < len(rs); i++ {
		// The transaction type and hash can be retrieved from the transaction itself
		if hasAutonityReceipt && i == len(txs) {
