...
func TestBlockEncoding(t *testing.T) {
	// this block now has baseFee = new(big.Int) since we do not allow baseFee = nil anymore
	blockEnc := common.FromHex("")
	var block Block
...
func TestHeaderEncodeDecodeJson(t *testing.T) {
	...
	t.Run("encode / decode with nil epoch in block header", func(t *testing.T) {
		header := &Header{
			ParentHash:        common.HexToHash("0a5843ac1cb04865017cb35a57b50b07084e5fcee39b5acadade33149f4fff9e"),
			UncleHash:         common.HexToHash("0a5843ac1c124732472342342387423897431293123020912dade33149f4fffe"),
			Coinbase:          common.HexToAddress("8888f1f195afa192cfee860698584c030f4c9db1"),
...
func TestEIP2718BlockEncoding(t *testing.T) {
	// this block now has baseFee = new(big.Int) since we do not allow baseFee = nil anymore
	blockEnc := common.FromHex("")
	var block Block
	if err := rlp.DecodeBytes(blockEnc, &block); err != nil {
		t.Fatal("decode error: ", err)
	}
...
func headerWithQuorumCertificate(quorumCertificate *AggregateSignature) *Header {
	return &Header{
		...
		// need to assign these fields because rlp decode will instantiate them
...
func TestQuorumCertificateDeserialization(t *testing.T) {
	...
	header = headerWithQuorumCertificate(&AggregateSignature{Signature: sig.(*blst.BlsSignature), Signers: &QuorumSigners{}})
	b, err = rlp.EncodeToBytes(header)
	require.NoError(t, err)
	headerDecoded = &Header{}
	err = rlp.Decode(bytes.NewReader(b), headerDecoded)
	...
	require.NoError(t, err)
	headerDecoded.QuorumCertificate.Signers.AssignPower(make(map[int]*big.Int), new(big.Int))
	require.Equal(t, header, headerDecoded)
}
...
func TestActivityProofDeserialization(t *testing.T) {
	...
	// empty signers is not allowed
	header = headerWithActivityProof(&AggregateSignature{Signature: sig.(*blst.BlsSignature), Signers: &QuorumSigners{}}, 4)
	b, err = rlp.EncodeToBytes(header)
	require.NoError(t, err)
	headerDecoded = &Header{}
	err = rlp.Decode(bytes.NewReader(b), headerDecoded)
	...
	require.NoError(t, err)
	headerDecoded.ActivityProof.Signers.AssignPower(make(map[int]*big.Int), new(big.Int))
	require.Equal(t, header, headerDecoded)
}

func headerWithEpoch(epoch *Epoch) *Header {
	return &Header{
		MixDigest: BFTDigest,
		Epoch:     epoch,
		// need to assign these fields because rlp decode will instantiate them
...
func TestEpochDeserialization(t *testing.T) {
	// if nil when encoded, should be nil when decoded
	header := headerWithEpoch(nil)
	b, err := rlp.EncodeToBytes(header)
	require.NoError(t, err)
	headerDecoded := &Header{}
	err = rlp.Decode(bytes.NewReader(b), headerDecoded)
	require.NoError(t, err)
	require.Nil(t, headerDecoded.Epoch)
	require.Equal(t, header, headerDecoded)
	...
	require.NoError(t, err)
	require.Equal(t, header, headerDecoded)
	require.True(t, validEpoch.Equal(headerDecoded.Epoch))

	// committee should not be nil
	...
	// committee should have at least 1 member
	epoch = validEpoch.Copy()
	...
	// committee should have valid keys
	epoch = validEpoch.Copy()
	epoch.Committee.Members[1].ConsensusKeyBytes = make([]byte, 27)
	header = headerWithEpoch(epoch)
	b, err = rlp.EncodeToBytes(header)
	require.NoError(t, err)
	headerDecoded = &Header{}
