...
func TestQueue2(t *testing.T) {
	...
	if pool.queue.queued[from].Len() != 2 {
		t.Error("expected len(queue) == 2, got", pool.queue.queued[from].Len())
	}
}
...
func TestMissingNonce(t *testing.T) {
	...
	if pool.queue.queued[addr].Len() != 1 {
		t.Error("expected 1 queued transaction, got", pool.queue.queued[addr].Len())
	}
	if pool.all.Count() != 1 {
		t.Error("expected 1 total transactions, got", pool.all.Count())
	}
...
func TestDropping(t *testing.T) {
	...
	if pool.queue.queued[account].Len() != 3 {
		t.Errorf("queued transaction mismatch: have %d, want %d", pool.queue.queued[account].Len(), 3)
	}
	if pool.all.Count() != 6 {
	...
	if _, ok := pool.queue.queued[account].txs.items[tx10.Nonce()]; !ok {
		t.Errorf("funded queued transaction missing: %v", tx10)
	}
	if _, ok := pool.queue.queued[account].txs.items[tx11.Nonce()]; !ok {
		t.Errorf("funded queued transaction missing: %v", tx10)
	}
	if _, ok := pool.queue.queued[account].txs.items[tx12.Nonce()]; ok {
...
func TestPostponing(t *testing.T) {
	...
	if len(pool.queue.queued) != 0 {
		t.Errorf("queued accounts mismatch: have %d, want %d", len(pool.queue.queued), 0)
	}
	...
	if _, ok := pool.queue.queued[accs[0]].txs.items[txs[0].Nonce()]; ok {
	...
	for i, tx := range txs[1:100] {
		if i%2 == 1 {
			...
			if _, ok := pool.queue.queued[accs[0]].txs.items[tx.Nonce()]; !ok {
	...
	for i, tx := range txs[100:] {
		if i%2 == 1 {
			if _, ok := pool.queue.queued[accs[1]].txs.items[tx.Nonce()]; !ok {
...
func TestQueueAccountLimiting(t *testing.T) {
	...
	for i := uint64(1); i <= testTxPoolConfig.AccountQueue+5; i++ {
		...
		if i <= testTxPoolConfig.AccountQueue {
			if pool.queue.queued[account].Len() != int(i) {
				t.Errorf("tx %d: queue size mismatch: have %d, want %d", i, pool.queue.queued[account].Len(), i)
			}
		} else {
			if pool.queue.queued[account].Len() != int(testTxPoolConfig.AccountQueue) {
				t.Errorf("tx %d: queue limit mismatch: have %d, want %d", i, pool.queue.queued[account].Len(), testTxPoolConfig.AccountQueue)
			}
		}
	}
...
func TestPendingLimiting(t *testing.T) {
	...
	for i := uint64(0); i < testTxPoolConfig.AccountQueue+5; i++ {
		...
		if len(pool.queue.queued) != 0 {
			t.Errorf("tx %d: queue size mismatch: have %d, want %d", i, pool.queue.queued[account].Len(), 0)
		}
	}
...
func TestAllowedTxSize(t *testing.T) {
	...
	//
	// Transaction RLP encodes signature values (r,s) as minimal-length integers, so
	...
	mkTx := func(nonce uint64, dataLen uint64) *types.Transaction {
		...
		t.Fatalf("failed to generate tx with full-size signature after many attempts")
		return nil
	}

	const largeDataLength = uint64(txMaxSize - 200) // enough to have a 5 bytes RLP encoding of the data length number
	txWithLargeData := mkTx(0, largeDataLength)
	maxTxLengthWithoutData := uint64(txWithLargeData.Size()) - largeDataLength
	maxTxDataLength := uint64(txMaxSize) - maxTxLengthWithoutData

	// Try adding a transaction with maximal allowed size
	tx := mkTx(0, maxTxDataLength)
	if err := pool.addRemoteSync(tx); err != nil {
		t.Fatalf("failed to add transaction of size %d, close to maximal: %v", int(tx.Size()), err)
	}
	// Try adding a transaction with random allowed size
	if err := pool.addRemoteSync(mkTx(1, uint64(rand.Intn(int(maxTxDataLength+1))))); err != nil {
		t.Fatalf("failed to add transaction of random allowed size: %v", err)
	}
	// Try adding a transaction above maximum size by one
	if err := pool.addRemoteSync(mkTx(2, maxTxDataLength+1)); err == nil {
		t.Fatalf("expected rejection on slightly oversize transaction")
	}
	// Try adding a transaction above maximum size by more than one
	if err := pool.addRemoteSync(mkTx(2, maxTxDataLength+1+uint64(rand.Intn(10*txMaxSize)))); err == nil {
...
func TestSetCodeTransactions(t *testing.T) {
	...
	// Create the pool to test the status retrievals with
	statedb, _ := state.New(types.EmptyRootHash, state.NewDatabaseForTesting())
	blockchain := newTestBlockChain(params.TestConfigNoVerkle, 1000000, statedb, new(event.Feed))
