...
func (pool *LegacyPool) ValidateTxBasics(tx *types.Transaction) error {
	//todo(youssef): create a subscription for epoch changes that saves atomically the most recent minbasefee
	var minGasPrice *big.Int
	currentCoreHeight := pool.chain.CurrentBlock().Number.Uint64() + 1
	eip1559Params, err := pool.chain.Eip1559ParamsByHeight(currentCoreHeight)
	if err == nil {
		minGasPrice = eip1559Params.MinBaseFee
	} else {
