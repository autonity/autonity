...
type ValidationOptions struct {
	...
	MinGasPrice *big.Int // Minimum gas price required. Autonity specific.
}
...
func ValidateTransaction(tx *types.Transaction, head *types.Header, signer types.Signer, opts *ValidationOptions) error {
	...
	rules := opts.Config.Rules(head.Number, head.Difficulty.Sign() == 0)
	...
	if opts.MinGasPrice.Cmp(tx.GasPrice()) > 0 {
		return core.ErrGasPriceBelowMin
	}
