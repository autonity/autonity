...
func isReimbursable(msg *Message) bool {
	// messages created through RPC (eth_call) may have a nil value
	if msg.To == nil {
		return false
	}
	dest := *msg.To
	// resolve oracle votes first.
	if dest == params.OracleContractAddress {
	...
	if dest == params.AccountabilityContractAddress {
		method, err := generated.AccountabilityAbi.MethodById(msg.Data)
		if err != nil {
			return false
		}
		switch method.Name {
		case "handleMisbehaviour":
			return true
		case "handleInnocenceProof":
			return true
		case "handleAccusation":
			return true
		default:
			return false
		}
	...
	if st.evm.Config.NoBaseFee && msg.GasFeeCap.Sign() == 0 && msg.GasTipCap.Sign() == 0 {
		...
		if vmerr == nil && !contractCreation {
			...
			if reimbursable {
				...
				fee.Mul(fee, uint256.MustFromBig(msg.GasPrice))
				st.state.AddBalance(msg.From, fee, tracing.BalanceChangeUnspecified)
			}
		}
