...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	chainman, _ := NewBlockChain(db, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil, NewTxSenderCacher(), nil, FakeContractBackendProvider, log.Root())
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, &cacheConfig, params.TestChainConfig, ethash.NewFaker(), vm.Config{}, nil, NewTxSenderCacher(), nil, FakeContractBackendProvider, log.Root())
		if err != nil {
