...
func testSetHead(t *testing.T, tt *rewindTest) {
	...
	chain, err := NewBlockChain(db, nil, params.AllEthashProtocolChangesWithAutonity, engine, vm.Config{}, nil, new(TxSenderCacher), nil)
	if err != nil {
	...
	if tt.sidechainBlocks > 0 {
		...
		new(Genesis).MustCommit(sidedb)
		sideblocks, _ = GenerateChain(params.AllEthashProtocolChangesWithAutonity, genesis, engine, sidedb, tt.sidechainBlocks, func(i int, b *BlockGen) {
	...
	canondb := rawdb.NewMemoryDatabase()
	new(Genesis).MustCommit(canondb)
	canonblocks, _ := GenerateChain(params.AllEthashProtocolChangesWithAutonity, genesis, engine, canondb, tt.canonicalBlocks, func(i int, b *BlockGen) {
