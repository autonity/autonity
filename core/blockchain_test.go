...
import (
	...
	"github.com/autonity/autonity/core/vm/program"
...
func newCanonical(t *testing.T, engine consensus.Engine, n int, full bool, scheme string) (ethdb.Database, *Genesis, *BlockChain, error) {
	...
	var (
		genesis = &Genesis{
			...
			Config:  params.TestConfigOld,
		}
	)
	// Initialize a fresh chain with only a genesis block
	blockchain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), DefaultCacheConfigWithScheme(scheme), genesis, engine, vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, scheme string) {
	...
	genDb, _, blockchain2, err := newCanonical(t, ethash.NewFaker(), i, full, scheme)
	if err != nil {
...
func testBlockChainImport(chain types.Blocks, blockchain *BlockChain) error {
	for _, block := range chain {
		...
		res, statedb, _, err := blockchain.processor.ProcessFromCache(block, statedb, vm.Config{})
		if err != nil {
...
func testLastBlock(t *testing.T, scheme string) {
	genDb, _, blockchain, err := newCanonical(t, ethash.NewFaker(), 0, true, scheme)
	if err != nil {
...
func testExtendCanonical(t *testing.T, full bool, scheme string) {
	...
	_, _, processor, err := newCanonical(t, ethash.NewFaker(), length, full, scheme)
	if err != nil {
...
func testBrokenChain(t *testing.T, full bool, scheme string) {
	...
	genDb, _, blockchain, err := newCanonical(t, ethash.NewFaker(), 10, full, scheme)
	if err != nil {
		t.Fatalf("failed to make new canonical chain: %v", err)
	}
	defer blockchain.Stop()
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool, scheme string) {
	...
	genDb, _, blockchain, err := newCanonical(t, ethash.NewFullFaker(), 0, full, scheme)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
...
func testInsertNonceError(t *testing.T, full bool, scheme string) {
	doTest := func(i int) {
		// Create a pristine chain and database
		genDb, _, blockchain, err := newCanonical(t, ethash.NewFaker(), 0, full, scheme)
		if err != nil {
			t.Fatalf("failed to create pristine chain: %v", err)
		}
		defer blockchain.Stop()
...
func testFastVsFullChains(t *testing.T, scheme string) {
	...
	var (
		...
		funds   = big.NewInt(1000000000000000000) //nolint:typecheck
		gspec   = &Genesis{
			Config:  params.TestConfigOld,
	...
	_, blocks, receipts := GenerateChainWithGenesis(gspec, ethash.NewFullFaker(), 1024, func(i int, block *BlockGen) {
	...
	archive, _ := NewBlockChain(archiveDb, DefaultCacheConfigWithScheme(scheme), gspec, ethash.NewFullFaker(), vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, DefaultCacheConfigWithScheme(scheme), gspec, ethash.NewFullFaker(), vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, DefaultCacheConfigWithScheme(scheme), gspec, ethash.NewFullFaker(), vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	defer ancient.Stop()
	...
	for i := 0; i < len(blocks); i++ {
		num, hash, _ := blocks[i].NumberU64(), blocks[i].Hash(), blocks[i].Time()
...
func testLightVsFastVsFullChainHeads(t *testing.T, scheme string) {
	...
	archive, _ := NewBlockChain(archiveDb, &archiveCaching, gspec, ethash.NewFaker(), vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	fastDb := makeDb()
	defer fastDb.Close()
	fast, _ := NewBlockChain(fastDb, DefaultCacheConfigWithScheme(scheme), gspec, ethash.NewFaker(), vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	defer fast.Stop()
	...
	ancientDb := makeDb()
	defer ancientDb.Close()
	ancient, _ := NewBlockChain(ancientDb, DefaultCacheConfigWithScheme(scheme), gspec, ethash.NewFaker(), vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	defer ancient.Stop()
	...
	light, _ := NewBlockChain(lightDb, DefaultCacheConfigWithScheme(scheme), gspec, ethash.NewFaker(), vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
...
func testChainTxReorgs(t *testing.T, scheme string) {
	...
	blockchain, _ := NewBlockChain(db, DefaultCacheConfigWithScheme(scheme), gspec, ethash.NewFaker(), vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
...
func testLogReorgs(t *testing.T, scheme string) {
	var (
		...
		gspec  = &Genesis{Config: params.TestConfigOld, Alloc: types.GenesisAlloc{addr1: {Balance: big.NewInt(10000000000000000)}}}
		signer = types.LatestSigner(gspec.Config)
	)

	blockchain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), DefaultCacheConfigWithScheme(scheme), gspec, ethash.NewFullFaker(), vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	defer blockchain.Stop()
...
func testLogRebirth(t *testing.T, scheme string) {
	var (
		...
		blockchain, _ = NewBlockChain(rawdb.NewMemoryDatabase(), DefaultCacheConfigWithScheme(scheme), gspec, engine, vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	)
	defer blockchain.Stop()
...
func testSideLogRebirth(t *testing.T, scheme string) {
	...
	_, chain, _ := GenerateChainWithGenesis(gspec, ethash.NewFullFaker(), 2, func(i int, gen *BlockGen) {
		if i == 1 {
			gen.OffsetTime(-9) // higher block difficulty
		}
	})
	...
	genDb, sideChain, _ := GenerateChainWithGenesis(gspec, ethash.NewFullFaker(), 2, func(i int, gen *BlockGen) {
		if i == 1 {
	...
	newBlocks, _ := GenerateChain(gspec.Config, sideChain[len(sideChain)-1], ethash.NewFullFaker(), genDb, 1, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(newBlocks); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
	checkLogEvents(t, newLogCh, rmLogsCh, 0, 0)
...
func testCanonicalBlockRetrieval(t *testing.T, scheme string) {
	_, gspec, blockchain, err := newCanonical(t, ethash.NewFaker(), 0, true, scheme)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
...
func TestEIP155Transition(t *testing.T) {
	t.Skip("transition not supported with autonity") // Due to AC's recent opcodes
...
func testEIP155Transition(t *testing.T, scheme string) {
	...
	blockchain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), DefaultCacheConfigWithScheme(scheme), gspec, ethash.NewFaker(), vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	defer blockchain.Stop()
...
//
// https://github.com/autonity/autonity/pull/15941
...
func testBlockchainHeaderchainReorgConsistency(t *testing.T, scheme string) {
	...
	for i := 0; i < len(forks); i++ {
		parent, err := genesis.ToBlock(triedb.NewDatabase(genDb, triedb.HashDefaults))
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), DefaultCacheConfigWithScheme(scheme), genesis, engine, vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	if err != nil {
...
func TestTrieForkGC(t *testing.T) {
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), nil, genesis, engine, vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testLargeReorgTrieGC(t *testing.T, scheme string) {
	...
	chain, err := NewBlockChain(db, DefaultCacheConfigWithScheme(scheme), genesis, engine, vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testBlockchainRecovery(t *testing.T, scheme string) {
	...
	var (
		...
		address = crypto.PubkeyToAddress(key.PublicKey)
		funds   = big.NewInt(1000000000)
		gspec   = &Genesis{Config: params.TestConfigOld, Alloc: types.GenesisAlloc{address: {Balance: funds}}}
	)
	height := uint64(64)
	_, blocks, receipts := GenerateChainWithGenesis(gspec, ethash.NewFaker(), int(height), nil)
...
//
// Details at:
//   - https://github.com/autonity/autonity/issues/18977
//   - https://github.com/autonity/autonity/pull/18988
...
func testLowDiffLongChain(t *testing.T, scheme string) {
	...
	chain, err := NewBlockChain(diskdb, DefaultCacheConfigWithScheme(scheme), genesis, engine, vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int, mergePoint int) {
	// Generate a canonical chain to act as the main dataset
	chainConfig := *params.TestConfigOld
	var (
		engine = ethash.NewFaker()
		key, _ = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, engine, vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testInsertKnownChainData(t *testing.T, typ string, scheme string) {
	...
	chain, err := NewBlockChain(chaindb, DefaultCacheConfigWithScheme(scheme), genesis, engine, vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testInsertKnownChainDataWithMerging(t *testing.T, typ string, mergeHeight int) {
	// Copy the TestConfigOld so we can modify it during tests
	chainConfig := *params.TestConfigOld
	var (
		genesis = &Genesis{
			BaseFee: big.NewInt(params.InitialBaseFee),
	...
	chain, err := NewBlockChain(chaindb, nil, genesis, engine, vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func getLongAndShortChains(t *testing.T, scheme string) (*BlockChain, []*types.Block, []*types.Block, *Genesis, error) {
	// Generate a canonical chain to act as the main dataset
	engine := ethash.NewFaker()
	genesis := &Genesis{
		Config:  params.TestConfigOld,
...
func testReorgToShorterRemovesCanonMapping(t *testing.T, scheme string) {
	chain, canonblocks, sideblocks, _, err := getLongAndShortChains(t, scheme)
	if err != nil {
		t.Fatal(err)
	}
	defer chain.Stop()
...
func benchmarkLargeNumberOfValueToNonexisting(b *testing.B, numTxs, numBlocks int, recipientFn func(uint64) common.Address) {
	...
	for i := 0; i < b.N; i++ {
		// Import the shared chain and the original canonical one
		chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, engine, vm.Config{}, nil, FakeContractBackendProvider(b), log.Root())
		if err != nil {
...
func testDeleteCreateRevert(t *testing.T, scheme string) {
	...
	// Import the canonical chain
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), DefaultCacheConfigWithScheme(scheme), gspec, engine, vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testDeleteRecreateSlots(t *testing.T, scheme string) {
	...
	}, nil, FakeContractBackendProvider(t), log.Root())
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testEIP1559Transition(t *testing.T, scheme string) {
	...
	gspec.BaseFee = big.NewInt(1000000000)
	gspec.GasLimit = 4712388
...
func testSetCanonical(t *testing.T, scheme string) {
	...
	chain, err := NewBlockChain(diskdb, DefaultCacheConfigWithScheme(scheme), gspec, engine, vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func TestCreateThenDeletePostByzantium(t *testing.T) {
	testCreateThenDelete(t, params.TestConfigOld)
}
...
func TestTransientStorageReset(t *testing.T) {
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, engine, vmConfig, nil, FakeContractBackendProvider(t), log.Root())
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func TestEIP3651(t *testing.T) {
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, engine, vm.Config{Tracer: logger.NewMarkdownLogger(&logger.Config{}, os.Stderr).Hooks()}, nil, FakeContractBackendProvider(t), log.Root())
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	state, _ := chain.State()
	...
	// In Autonity with ethash, the coinbase is cold (2600 gas) instead of warm (100 gas) since
	// EIP-3651 (warm coinbase) doesn't apply without actual PoS merge consensus.
	expectedGas := params.TxGas + 5*vm.GasFastestStep + vm.GasQuickStep + params.ColdAccountAccessCostEIP2929 + innerGas
	if block.GasUsed() != expectedGas {
		t.Fatalf("incorrect amount of gas spent: expected %d, got %d", expectedGas, block.GasUsed())
	}
	state, _ = chain.State()
...
func TestEIP7702(t *testing.T) {
	t.Skip("EIP-7702 requires post-merge consensus which is not compatible with autonity ethash tests")
	var (
		config  = *params.TestConfigNoVerkle
		signer  = types.LatestSigner(&config)
		engine  = ethash.NewFaker()
...
func testChainReorgSnapSync(t *testing.T, ancientLimit uint64) {
	...
	chain, _ := NewBlockChain(db, DefaultCacheConfigWithScheme(rawdb.PathScheme), gspec, ethash.NewFaker(), vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	defer chain.Stop()
...
func testInsertChainWithCutoff(t *testing.T, cutoff uint64, ancientLimit uint64, genesis *Genesis, blocks []*types.Block, receipts []types.Receipts) {
	...
	db, _ := rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(), "", "", false)
	defer db.Close()
	chain, _ := NewBlockChain(db, DefaultCacheConfigWithScheme(rawdb.PathScheme), genesis, ethash.NewFaker(), vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	defer chain.Stop()
