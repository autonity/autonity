...
func testRepairWithScheme(t *testing.T, tt *rewindTest, snapshots bool, scheme string) {
	...
	if tt.sidechainBlocks > 0 {
		sideblocks, _ = GenerateChain(gspec.Config, chain.Genesis(), engine, rawdb.NewMemoryDatabase(), tt.sidechainBlocks, func(i int, b *BlockGen) {
	...
	canonblocks, _ := GenerateChain(gspec.Config, chain.Genesis(), engine, rawdb.NewMemoryDatabase(), tt.canonicalBlocks, func(i int, b *BlockGen) {
	...
	newChain, err := NewBlockChain(db, config, gspec, engine, vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	if err != nil {
...
// TestIssue23496 tests scenario described in https://github.com/autonity/autonity/pull/23496#issuecomment-926393893
...
func testIssue23496(t *testing.T, scheme string) {
	...
	chain, err := NewBlockChain(db, DefaultCacheConfigWithScheme(scheme), gspec, engine, vm.Config{}, nil, FakeContractBackendProvider(t), log.Root())
	if err != nil {
		t.Fatalf("Failed to create chain: %v", err)
	}
