...
func testRepair(t *testing.T, tt *rewindTest, snapshots bool) {
	...
	newChain, err := NewBlockChain(db, nil, params.TestChainConfig, engine, vm.Config{}, nil, new(TxSenderCacher), nil, nil, log.Root())
	if err != nil {
...
func TestIssue23496(t *testing.T) {
	...
	chain, err = NewBlockChain(db, nil, params.TestChainConfig, engine, vm.Config{}, nil, NewTxSenderCacher(), nil, nil, log.Root())
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	defer chain.Stop()
