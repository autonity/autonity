...
func ApplyTransaction(config *params.ChainConfig, bc ChainContext, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, cfg vm.Config, contract *autonity.Contract) (*types.Receipt, uint64, error) {
	...
	_, gas, failed, err := ApplyMessage(vmenv, msg, gp, contract)
	if err != nil {
		return nil, 0, err
	}
