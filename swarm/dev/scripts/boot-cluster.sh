...
#
# The cluster contains a bootnode, a autonity node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the autonity and
# swarm nodes
start_bootnode() {
...
# start_autonity_node starts a autonity node with --datadir pointing at <base-dir>/autonity
# and a single, unlocked account with password "autonity"
start_autonity_node() {
  local dir="${base_dir}/autonity"
  mkdir -p "${dir}"

  local password="autonity"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating autonity account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get autonity account address"
  fi
  ...
  start_node "autonity" "${GETH_IP}" "$(which autonity)" ${args[@]}
}
...
create_account() {
  ...
  autonity --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
