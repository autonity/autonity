...
func (r *testerChainReader) Config() *params.ChainConfig                 { return params.AllSomaProtocolChanges }
...
func TestVoting(t *testing.T) {
	...
	for i, tt := range tests {
		...
		// Pass all the headers through soma and ensure tallying succeeds
		head := headers[len(headers)-1]

		snap, err := New(&params.SomaConfig{Epoch: tt.epoch}, db).snapshot(&testerChainReader{db: db}, head.Number.Uint64(), head.Hash(), headers)
		if err != nil {
