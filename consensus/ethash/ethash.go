...
// return an error as sealing is not supported anymore, except in fake modes
// where it immediately returns the block as sealed for testing purposes.
func (ethash *Ethash) Seal(parentHeader *types.Header, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	// In fake mode, return a 0 nonce immediately (for testing)
	// Real PoW mining is no longer supported in Autonity
	header := block.Header()
