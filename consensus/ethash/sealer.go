...
func (ethash *Ethash) Seal(parentHeder *types.Header, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	if ethash.shared != nil {
		return ethash.shared.Seal(parentHeder, block, results, stop)
	}
	...
	go func() {
		...
		case <-ethash.update:
			...
			close(abort)
			if err := ethash.Seal(parentHeder, block, results, stop); err != nil {
