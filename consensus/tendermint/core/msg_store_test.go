...
func TestMsgStore(t *testing.T) {
	...
	t.Run("save equivocation msgs in msg store", func(t *testing.T) {
		ms := NewMsgStore()
		preVoteNil := message.NewPrevote(round, height, types.NilValue, makeSigner(proposerKey), &committee.Members[proposerIdx], cSize)
		ms.Save(preVoteNil)
		...
		require.Equal(t, common.Big1, ms.PrevotesPowerFor(height, round, types.NilValue))
	...
	t.Run("Save aggregated votes in msg store", func(t *testing.T) {
		...
		for _, member := range committee.Members {
			m := member
			preVoteNil := message.NewPrevote(round, height, types.NilValue, makeSigner(keys[member.Address].consensus), &m, cSize)
		...
		for i, member := range committee.Members {
			...
			votes := ms.GetPrevotes(height, func(msg *message.Prevote) bool {
				return msg.R() == round && msg.Value() == types.NilValue && msg.Signers().Contains(int(m.Index))
			})
		...
		votes := ms.GetPrevotes(height, func(m *message.Prevote) bool {
			return m.R() == round && m.Value() == types.NilValue
		})

		require.Equal(t, 1, len(votes))
		require.Equal(t, common.Big5, ms.PrevotesPowerFor(height, round, types.NilValue))
	})

	t.Run("query a presented preVote from msg store", func(t *testing.T) {
		ms := NewMsgStore()
		preVote := message.NewPrevote(round, height, types.NilValue, makeSigner(proposerKey), &committee.Members[proposerIdx], cSize)
		ms.Save(preVote)

		votes := ms.GetPrevotes(height, func(m *message.Prevote) bool {
			return m.R() == round && m.Value() == types.NilValue && m.Signers().Contains(proposerIdx)
		})
		...
		assert.Equal(t, types.NilValue, votes[0].Value())
	})
	...
	t.Run("delete msgs at a specific height", func(t *testing.T) {
		...
		require.Equal(t, uint64(0), ms.PrevotesPowerFor(height, round, types.NilValue).Uint64())
	})
	...
	t.Run("SearchQuorum correctly detects quorum of prevotes", func(t *testing.T) {
		...
		require.Equal(t, 0, len(ms.SearchQuorum(height, round, types.NilValue, common.Big1)))
		...
		require.Equal(t, 1, len(ms.SearchQuorum(height, round, types.NilValue, common.Big1)))
		require.Equal(t, preVoteNotNil.Hash(), ms.SearchQuorum(height, round, types.NilValue, common.Big1)[0].Hash())

		preVoteNotNil = message.NewPrevote(round, height, notNilValue, makeSigner(keyBob), &committee.Members[indexBob], cSize)
		ms.Save(preVoteNotNil)

		require.Equal(t, 2, len(ms.SearchQuorum(height, round, types.NilValue, common.Big1)))
		require.Equal(t, 2, len(ms.SearchQuorum(height, round, types.NilValue, common.Big2)))
		require.Equal(t, 0, len(ms.SearchQuorum(height, round, types.NilValue, common.Big3)))
