...
func TestTendermintOnQuorumPrevote(t *testing.T) {
	t.Run("Line36 to Line43, on prevote for the first time.", func(t *testing.T) {
		...
		initState(core, committeeSet, currentHeight, roundProposed, nil, int64(-1), nil, int64(-1), prevote, false)
		...
		receivedPrevoteMsg := Message{
			...
			power:   core.CommitteeSet().Quorum(),
		}
		...
		// subscribe for checking the broadcast msg: precommit for locked Value on the specific view.
		checkBroadcastMsg(t, backendMock, core, msgPrecommit, currentHeight, roundProposed, proposalBlock.Hash())
	...
	t.Run("On condition line36, Only line42, line43 was executed with step > prevote.", func(t *testing.T) {
		...
		lockedValue := generateBlock(currentHeight) // locked at a different value.
		clientAddr := currentCommittee[0].Address
		...
		initState(core, committeeSet, currentHeight, roundProposed, lockedValue, 0, nil, int64(-1), precommit, false)
		core.sentPrecommit = true

		// condition 2f+1 <PREVOTE, h_p, round_p, id(v)>, power of pre-vote. line 36.
		// prepare the received prevote msgs and init the round state.
		receivedPrevoteMsg := Message{
			Code:    msgPrevote,
			Address: currentCommittee[2].Address,
			power:   core.CommitteeSet().Quorum(),
		...
		checkState(t, core, currentHeight, roundProposed, lockedValue, int64(0), proposalBlock, roundProposed, precommit)
	})
}
