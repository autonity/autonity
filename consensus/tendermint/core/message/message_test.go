...
func TestComplexAggregation(t *testing.T) {
	...
	t.Run("aggregate same message", func(t *testing.T) {
		var votes1 []Vote
		votes1 = append(votes1, NewPrevote(r, h, v1, defaultSigner, &testCommittee[0], csize))
		votes1 = append(votes1, NewPrevote(r, h, v1, defaultSigner, &testCommittee[1], csize))
		aggregate1 := AggregatePrevotes(votes1)
		require.Equal(t, 2, len(aggregate1.Signers().Flatten()))

		var votes2 []Vote
		votes2 = append(votes2, NewPrevote(r, h, v1, defaultSigner, &testCommittee[0], csize))
		votes2 = append(votes2, NewPrevote(r, h, v1, defaultSigner, &testCommittee[2], csize))
