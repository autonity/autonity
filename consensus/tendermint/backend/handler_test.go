...
func TestFutureHeightMessage(t *testing.T) {
	t.Run("received future height message is buffered", func(t *testing.T) {
		...
		require.Equal(t, 1, len(backend.messages[futureHeight]))
		require.Equal(t, data.Hash(), backend.messages[futureHeight][0].Message.Hash())
		require.Equal(t, futureHeight, backend.maxHeight)
		require.Equal(t, uint64(1), backend.size)
	})
	t.Run("if future message buffer is full, messages farther in the future are dropped", func(t *testing.T) {
		...
		backend.future.RLock()
		defer backend.future.RUnlock()
		require.Equal(t, maxFutureMsgs, len(backend.messages))
		require.Equal(t, uint64(maxFutureMsgs), backend.size) // works because we send only one message per height
	})
	t.Run("When processing future height messages, future height messages are re-injected", func(t *testing.T) {
		...
		backend.future.RLock()
		require.Equal(t, uint64(4), backend.size)
		backend.future.RUnlock()

		backend.ProcessFutureMsgs(1)

		backend.future.RLock()
		require.Equal(t, uint64(0), backend.size)
		backend.future.RUnlock()
	})
}
