...
var (
	...
	ErrNotFuture = errors.New("message has not future height anymore")
...
func (sb *Backend) saveFutureMsg(msg message.Msg, errCh chan<- error, sender common.Address) error {
	...
	sb.messages[h] = append(sb.messages[h], e)
	sb.size++

	// if needed, drop heights until we are back under the threshold
	for sb.size > maxFutureMsgs {
		maxHeightEvs, ok := sb.messages[sb.maxHeight]
		sb.logger.Debug("deleting excess future height messages", "height", sb.maxHeight)
		if ok {
			sb.size -= uint64(len(maxHeightEvs))
			...
			delete(sb.messages, sb.maxHeight)
		}
		...
		if sb.maxHeight < sb.minHeight-1 {
...
func (sb *Backend) ProcessFutureMsgs(height uint64) {
	...
	if sb.size == 0 || sb.minHeight > height {
		return
	}

	// process future messages up to current height
	for h := sb.minHeight; h <= height; h++ {
		evs, ok := sb.messages[h]
		// there might be holes in heights in the future messages
		if ok {
			sb.logger.Debug("processing future height messages", "height", h, "n", len(sb.messages[h]))
			for _, e := range evs {
				sb.handleDecodedMsg(e.Message, e.ErrCh, e.Sender)
				sb.size--
			}
			delete(sb.messages, h)
		}
	}
	...
	sb.minHeight = height + 1
	sb.lastProcessedHeight = height
}
