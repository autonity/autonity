...
func (sb *Backend) verifySigner(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	committee, err := sb.retrieveCommittee(header, parents, chain)
...
func (sb *Backend) verifyCommittedSeals(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	validators := validator.NewSet(committee, sb.config.GetProposerPolicy())
	...
	validSeal := 0
	proposalSeal := tendermintCore.PrepareCommittedSeal(header.Hash(), header.Round, header.Number)
...
func (sb *Backend) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header) {
	...
	validators, err := sb.getCommittee(header, chain, state)
	if err != nil {
		sb.logger.Error("finalize. after getCommittee", "err", err.Error())
		return
	}
...
func (sb *Backend) FinalizeAndAssemble(chain consensus.ChainReader, header *types.Header, statedb *state.StateDB, txs []*types.Transaction,
	...
	validators, err := sb.getCommittee(header, chain, statedb)
	if err != nil {
		sb.logger.Error("FinalizeAndAssemble. after getCommittee", "err", err.Error())
		return nil, err
	}
...
func (sb *Backend) getCommittee(header *types.Header, chain consensus.ChainReader, state *state.StateDB) (types.Committee, error) {
	sb.contractsMu.Lock()
	defer sb.contractsMu.Unlock()
	var committee types.Committee

	if header.Number.Int64() == 1 {
		sb.blockchain.GetAutonityContract().SavedCommitteeRetriever = func(i uint64) (types.Committee, error) {
			chain := chain
			return sb.retrieveSavedCommittee(i, chain)
		}
		...
		committee, err = sb.retrieveSavedCommittee(1, chain)
		if err != nil {
			return nil, err
		}
		...
		var err error
		committee, err = sb.blockchain.GetAutonityContract().ContractGetCommittee(chain, header, state)
		if err != nil {
			sb.logger.Error("ContractGetCommittee returns err", "err", err)
			return nil, err
		}
	}
...
func (sb *Backend) retrieveSavedCommittee(number uint64, chain consensus.ChainReader) (types.Committee, error) {
	if number == 0 {
		number = 1
	}
...
func (sb *Backend) retrieveCommittee(header *types.Header, parents []*types.Header, chain consensus.ChainReader) (types.Committee, error) {

	var committee types.Committee
	var err error

	/*
		We can't use retrieveSavedCommittee if parents are being passed :
	...
	*/
	...
	if len(parents) > 0 {
		...
		committee, err = sb.retrieveSavedCommittee(header.Number.Uint64(), chain)
	}
