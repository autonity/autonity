...
func (sb *Backend) verifyCommittedSeals(header, parent *types.Header) error {
	...
	votingPower := make(map[common.Address]uint64, len(parent.Committee))
	var committeeVotingPower uint64
	for _, member := range parent.Committee {
		votes[member.Address] = 0
		v := member.VotingPower.Uint64()
		votingPower[member.Address] = v
		committeeVotingPower += v
	}

	// Check whether the committed seals were generated by committee members defined in parent.
