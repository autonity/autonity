...
func TestAccusationVerifier(t *testing.T) {
	...
	t.Run("Test validate accusation, with invalid signature of msg", func(t *testing.T) {
		...
		newProposal := newProposalMessage(height, 1, 0, keys[invalidCommittee.Members[0].Address], invalidCommittee, nil)
...
func TestMisbehaviourVerifier(t *testing.T) {
	...
	t.Run("Test validate misbehaviour Proof, with invalid signature of evidence msgs", func(t *testing.T) {
		...
		p.Message = proposal
		invalidPreCommit := newVoteMsg(height, 1, proto.MsgPrecommit, ikeys[invalidCommittee.Members[0].Address], proposal.Value(), invalidCommittee)
		p.Evidences = append(p.Evidences, invalidPreCommit)

		lastHeader := newBlockHeader(lastHeight, nil)
		epochHeader := newBlockHeader(common.Big1.Uint64(), committee)
		chainMock := NewMockChainContext(ctrl)
		chainMock.EXPECT().EpochHeadAndParentHead(height).AnyTimes().Return(epochHeader, lastHeader, nil)
		mv := MisbehaviourVerifier{chain: chainMock}
		ret := mv.validateProof(&p)
		assert.Equal(t, failureResult, ret)
	})
...
func TestCheckMsgSignature(t *testing.T) {
	...
	t.Run("normal case, proposal msg is checked correctly", func(t *testing.T) {
		...
		lastHeader := newBlockHeader(lastHeight, nil)
		epochHeader := newBlockHeader(common.Big1.Uint64(), committee)
		proposal := newProposalMessage(height, round, -1, keys[committee.Members[0].Address], committee, nil)
		chainMock := NewMockChainContext(ctrl)
		chainMock.EXPECT().EpochHeadAndParentHead(height).Return(epochHeader, lastHeader, nil)
	...
	t.Run("a future msg is received, expect an error of errFutureMsg", func(t *testing.T) {
		...
		proposal := newProposalMessage(futureHeight, round, -1, keys[committee.Members[0].Address], committee, nil)
		chainMock := NewMockChainContext(ctrl)
		chainMock.EXPECT().EpochHeadAndParentHead(futureHeight).Return(nil, nil, errFutureMsg)
	...
	t.Run("chain cannot provide the last header of the height that msg votes on, expect an error of errFutureMsg", func(t *testing.T) {
		...
		proposal := newProposalMessage(height, round, -1, keys[committee.Members[0].Address], committee, nil)
		chainMock := NewMockChainContext(ctrl)
		chainMock.EXPECT().EpochHeadAndParentHead(height).Return(nil, nil, errFutureMsg)
		require.Equal(t, errFutureMsg, checkMsgSignature(chainMock, proposal))
	})
...
func TestCheckEquivocation(t *testing.T) {
	...
	t.Run("check equivocation with valid Proof of equivocation", func(t *testing.T) {
		proposal := newProposalMessage(height, round, -1, keys[committee.Members[0].Address], committee, nil)
		vote1 := newVoteMsg(height, round, proto.MsgPrevote, keys[committee.Members[0].Address], proposal.Value(), committee)
		vote2 := newVoteMsg(height, round, proto.MsgPrevote, keys[committee.Members[0].Address], nilValue, committee)
