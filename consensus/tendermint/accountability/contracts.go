...
func (a *AccusationVerifier) Run(input []byte, blockNumber uint64, evm *vm.EVM) ([]byte, error) {
	if len(input) <= 32 {
...
func (c *MisbehaviourVerifier) Run(input []byte, _ uint64, evm *vm.EVM) ([]byte, error) {
	if len(input) <= 32 {
		return failureReturn, nil
	}
	// the 1st 32 bytes are length of bytes array in solidity, take RLP bytes after it.
...
func (c *InnocenceVerifier) Run(input []byte, blockNumber uint64, evm *vm.EVM) ([]byte, error) {
