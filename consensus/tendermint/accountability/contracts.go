...
func verifyAccusation(p *Proof, committee *types.Committee) bool {
	...
	case autonity.PVN:
		if p.Message.Code() != message.PrevoteCode || p.Message.Value() == types.NilValue {
			return false
		}
	...
	case autonity.C1:
		if p.Message.Code() != message.PrecommitCode || p.Message.Value() == types.NilValue {
			return false
		}
...
func (c *MisbehaviourVerifier) validMisbehaviourOfPN(p *Proof, committee *types.Committee) bool {
	...
	if committee.Members[p.OffenderIndex].Address == proposal.Signer() &&
		preCommit.R() < proposal.R() && preCommit.Value() != types.NilValue {
		return true
	}
	return false
}
...
func (c *MisbehaviourVerifier) validMisbehaviourOfPO(p *Proof, committee *types.Committee) bool {
	...
	case *message.Precommit:
		if vote.R() == proposal.ValidRound() &&
			...
			vote.Value() != types.NilValue && vote.Value() != proposal.Value() {
			return true
		}
		if vote.R() > proposal.ValidRound() &&
			vote.R() < proposal.R() &&
			vote.Signers().Contains(p.OffenderIndex) && committee.Members[p.OffenderIndex].Address == proposal.Signer() &&
			vote.Value() != types.NilValue {
			return true
		}
...
func (c *MisbehaviourVerifier) validMisbehaviourOfPVN(p *Proof) bool {
	...
	if !present || prevote.Code() != message.PrevoteCode || prevote.Value() == types.NilValue {
		return false
	}
	...
	if len(preCommits) == 1 {
		...
		return pc.R()+1 == prevote.R() && pc.Value() != types.NilValue && pc.Value() != prevote.Value()
	}
	...
	if p.DistinctPrecommits.Len() > 0 {
		...
		for i, pc := range preCommits {
			...
			// preCommit at R'
			if i == 0 {
				if pc.Value == types.NilValue || pc.Value == prevote.Value() {
					return false
				}
			} else {
				// preCommits at between R' and R-1, they should be nil.
				if pc.Value != types.NilValue {
					return false
				}
			}
...
func (c *MisbehaviourVerifier) validMisbehaviourOfPVO(p *Proof, committee *types.Committee) bool {
	...
	for _, pv := range p.Evidences[1:] {
		...
		if pv.Code() != message.PrevoteCode || pv.R() != validRound || pv.Value() == types.NilValue ||
...
func (c *MisbehaviourVerifier) validMisbehaviourOfPVO12(p *Proof) bool {
	...
	if len(precommits) == 1 {
		...
		return preC.Value() != prevote.Value() && preC.Value() != types.NilValue
	}
	...
	if p.DistinctPrecommits.Len() > 0 {
		...
		for _, pc := range allPreCommits {
			...
			if pc.Value != prevote.Value() && pc.Value != types.NilValue && pc.Round > lastRoundForNotV {
...
func (c *MisbehaviourVerifier) validMisbehaviourOfC(p *Proof, committee *types.Committee) bool {
	...
	if !present || preCommit.Code() != message.PrecommitCode || preCommit.Value() == types.NilValue {
		return false
	}
...
func validInnocenceProofOfPVN(p *Proof) bool {
	...
	if !(preVote.Code() == message.PrevoteCode && preVote.Value() != types.NilValue) {
		return false
	}
...
func validInnocenceProofOfC1(p *Proof, committee *types.Committee) bool {
	...
	if preCommit.Value() == types.NilValue {
		return false
	}
