...
func TestRuleEngine(t *testing.T) {
	...
	t.Run("RunRule to address misbehaviour of rule PVO1, node last precommited at a value of not v", func(t *testing.T) {
		...
		pcValidRound := newVoteMsg(height, 0, msgPrecommit, maliciousNode, oldProposal.Value(), committee)
		_, err = fd.msgStore.Save(pcValidRound)
		assert.NoError(t, err)

		// simulate a precommit at r: 1 with value v.
		preCommitForV := newVoteMsg(height, 1, msgPrecommit, maliciousNode, oldProposal.Value(), committee)
		...
		for _, p := range onChainProofs {
			if p.Type == autonity.Misbehaviour && p.Rule == PVO1 {
				presentPVO1 = true
