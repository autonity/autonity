...
func TestContractsManagement(t *testing.T) {
	...
	LoadAccountabilityPreCompiledContracts(nil)
	...
	assert.Nil(t, vm.PrecompiledContractsBerlin[checkInnocenceAddress])
	assert.Nil(t, vm.PrecompiledContractsBerlin[checkAccusationAddress])
	assert.Nil(t, vm.PrecompiledContractsBerlin[checkMisbehaviourAddress])

	assert.Nil(t, vm.PrecompiledContractsBLS[checkInnocenceAddress])
	assert.Nil(t, vm.PrecompiledContractsBLS[checkAccusationAddress])
	assert.Nil(t, vm.PrecompiledContractsBLS[checkMisbehaviourAddress])
}
...
func TestAccusationVerifier(t *testing.T) {
	...
	t.Run("Test validate accusation, with wrong rule ID", func(t *testing.T) {
		...
		av := AccusationVerifier{}
		assert.Equal(t, failure128Byte, av.validateAccusation(&p))
	})
	...
	t.Run("Test validate accusation, with correct accusation msg", func(t *testing.T) {
		...
		liteProposal := newProposal.ToLiteProposal()
		p.Message = liteProposal
		...
		ret := av.validateAccusation(&p)
		assert.NotEqual(t, failure128Byte, ret)
		assert.Equal(t, common.LeftPadBytes(proposer.Bytes(), 32), ret[0:32])
		assert.Equal(t, liteProposal.MsgHash().Bytes(), ret[32:64])
		assert.Equal(t, validByte, ret[64:96])
	})
	...
	t.Run("Test validate accusation, with PVO accusation msgs", func(t *testing.T) {
		...
		p.Message = preVote
		p.Evidence = append(p.Evidence, oldProposal.ToLiteProposal())
		lastHeader := newBlockHeader(lastHeight, committee)
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		chainMock := NewMockBlockChainContext(ctrl)
...
func TestMisbehaviourVerifier(t *testing.T) {
	...
	t.Run("Test validate misbehaviour AccountabilityProof, with invalid signature of misbehaved msg", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		var p AccountabilityProof
		p.Rule = autonity.PO
	...
	t.Run("Test validate misbehaviour AccountabilityProof, with invalid signature of evidence msgs", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		var p AccountabilityProof
		p.Rule = autonity.PO
	...
	t.Run("Test validate misbehaviour AccountabilityProof of PN rule with correct AccountabilityProof", func(t *testing.T) {
		// prepare a AccountabilityProof that node proposes for a new value, but he preCommitted a non nil value
		// at previous rounds, such AccountabilityProof should be valid.
		var p AccountabilityProof
		p.Rule = autonity.PN
		proposal := newProposalMessage(height, 1, -1, proposerKey, committee, nil)
		p.Message = proposal.ToLiteProposal()
	...
	t.Run("Test validate misbehaviour AccountabilityProof of PN rule with incorrect proposal of AccountabilityProof", func(t *testing.T) {
	...
	t.Run("Test validate misbehaviour AccountabilityProof of PN rule with no evidence of AccountabilityProof", func(t *testing.T) {
		var p AccountabilityProof
		p.Rule = autonity.PN
		proposal := newProposalMessage(height, 1, -1, proposerKey, committee, nil)
		p.Message = proposal
	...
	t.Run("Test validate misbehaviour AccountabilityProof of PO, propose a v rather than the locked one", func(t *testing.T) {
	...
	t.Run("Test validate misbehaviour AccountabilityProof of PO, propose a valid round rather than the locked one", func(t *testing.T) {
	...
	t.Run("Test validate misbehaviour AccountabilityProof of PO, propose a different value rather than the one that have quorum "+
		...
		for _, c := range committee {
			...
			p.Evidence = append(p.Evidence, preVotes)
		}
		lastHeader := newBlockHeader(lastHeight, committee)
		chainMock := NewMockBlockChainContext(ctrl)
		chainMock.EXPECT().GetHeaderByNumber(lastHeight).Return(lastHeader)
	...
	t.Run("Test validate misbehaviour AccountabilityProof of PO, with no evidence", func(t *testing.T) {
		var p AccountabilityProof
		p.Rule = autonity.PO
		proposal := newProposalMessage(height, 3, 0, proposerKey, committee, nil)
		p.Message = proposal
	...
	t.Run("Test validate misbehaviour AccountabilityProof of PO, with a proposal of new value", func(t *testing.T) {
		var p AccountabilityProof
		p.Rule = autonity.PO
	...
	t.Run("Test validate misbehaviour AccountabilityProof of PVN rule, with correct AccountabilityProof", func(t *testing.T) {
		...
		p.Message = preVote
		p.Evidence = append(p.Evidence, proposal.ToLiteProposal(), preCommit, preCommitR1, preCommitR2)
	...
	t.Run("Test validate misbehaviour AccountabilityProof of PVN rule, with gaps in preCommits", func(t *testing.T) {
		// simulate a p of misbehaviour of PVN, with the node preVote for V1, but he preCommit
		// at a different value V2 at previous round. The validation of the misbehaviour p should
		// return ture.
		var p AccountabilityProof
		...
		preVote := newVoteMsg(height, 3, proto.MsgPrevote, proposerKey, proposal.Value(), committee)
		p.Message = preVote
		p.Evidence = append(p.Evidence, preCommit, preCommitR1)
	...
	t.Run("Test validate misbehaviour AccountabilityProof of PVN rule, with no evidence", func(t *testing.T) {
		var p AccountabilityProof
		p.Rule = autonity.PVN
		proposal := newProposalMessage(height, 3, -1, proposerKey, committee, nil)
		// node preVote for V2 at round 3
	...
	t.Run("Test validate misbehaviour AccountabilityProof of PVN rule, with wrong msg", func(t *testing.T) {
		var p AccountabilityProof
		p.Rule = autonity.PVN
		proposal := newProposalMessage(height, 3, -1, proposerKey, committee, nil)
	...
	t.Run("Test validate misbehaviour AccountabilityProof of PVN rule, with wrong preVote value", func(t *testing.T) {
		var p AccountabilityProof
		p.Rule = autonity.PVN
		// node locked at V1 at round 0.
		preCommit := newVoteMsg(height, 0, proto.MsgPrecommit, proposerKey, noneNilValue, committee)
	...
	t.Run("Test validate misbehaviour proof of PVO12 rule, with correct proof", func(t *testing.T) {
		...
		p.Message = preVote
		p.Evidence = append(p.Evidence, correspondingProposal.ToLiteProposal(), pcForV, pcForNotV)
		mv := MisbehaviourVerifier{}
		ret := mv.validProof(&p)
		assert.Equal(t, true, ret)
	})
	...
	t.Run("Test validate misbehaviour proof of PVO12 rule, with precommits out of round range", func(t *testing.T) {
		correspondingProposal := newProposalMessage(height, 3, 0, proposerKey, committee, nil)
	...
	t.Run("Test validate misbehaviour proof of PVO rule, with correct proof", func(t *testing.T) {
		...
		p.Evidence = append(p.Evidence, correspondingProposal.ToLiteProposal())
	...
	t.Run("Test validate misbehaviour proof of PVO12 rule, with precommits at round VR ", func(t *testing.T) {
		correspondingProposal := newProposalMessage(height, 3, 0, proposerKey, committee, nil)
		// a precommit at round 0 (vr) for V. Including this will cause the proof to be invalid.
	...
	t.Run("Test validate misbehaviour AccountabilityProof of C rule, with correct AccountabilityProof", func(t *testing.T) {
	...
	t.Run("Test validate misbehaviour AccountabilityProof of C rule, with no Evidence", func(t *testing.T) {
		var p AccountabilityProof
		p.Rule = autonity.C
		// Node preCommit for V at round R.
		preCommit := newVoteMsg(height, 0, proto.MsgPrecommit, proposerKey, noneNilValue, committee)
	...
	t.Run("Test validate misbehaviour AccountabilityProof of C rule, with wrong preCommit msg", func(t *testing.T) {
	...
	t.Run("Test validate misbehaviour AccountabilityProof of C rule, with wrong msg", func(t *testing.T) {
		var p AccountabilityProof
		p.Rule = autonity.C
	...
	t.Run("Test validate misbehaviour AccountabilityProof of C rule, with invalid evidence", func(t *testing.T) {
	...
	t.Run("Test validate misbehaviour AccountabilityProof of C rule, with invalid evidence: duplicated msg in evidence", func(t *testing.T) {
		// the evidence contains same value of preCommit that node preVoted for.
		var p AccountabilityProof
		p.Rule = autonity.C
	...
	t.Run("Test validate misbehaviour AccountabilityProof of C rule, with invalid evidence: no quorum preVotes", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		var p AccountabilityProof
		p.Rule = autonity.C
...
func TestInnocenceVerifier(t *testing.T) {
	...
	t.Run("Test validate innocence AccountabilityProof with invalid signature of message", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		var p AccountabilityProof
		p.Rule = autonity.PO
	...
	t.Run("Test validate innocence AccountabilityProof, with invalid signature of evidence msgs", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		var p AccountabilityProof
		p.Rule = autonity.PO
	...
	t.Run("Test validate innocence AccountabilityProof of PO rule, with wrong msg", func(t *testing.T) {
		var p AccountabilityProof
		p.Rule = autonity.PO
	...
	t.Run("Test validate innocence AccountabilityProof of PVN rule, with wrong msg", func(t *testing.T) {
		var p AccountabilityProof
		p.Rule = autonity.PVN
	...
	t.Run("Test validate innocence AccountabilityProof of PVN rule, with a wrong preVote for nil", func(t *testing.T) {
		var p AccountabilityProof
		p.Rule = autonity.PVN
	...
	t.Run("Test validate innocence AccountabilityProof of PVN rule, with no evidence", func(t *testing.T) {
		var p AccountabilityProof
		p.Rule = autonity.PVN
		wrongMsg := newVoteMsg(height, 1, proto.MsgPrevote, proposerKey, noneNilValue, committee)
		p.Message = wrongMsg
	...
	t.Run("Test validate innocence AccountabilityProof of PVN rule, with a corresponding proposal", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		...
		p.Rule = autonity.PVN
		proposal := newProposalMessage(height, 1, -1, proposerKey, committee, nil)
		p.Evidence = append(p.Evidence, proposal.ToLiteProposal())
	...
	t.Run("Test validate innocence proof of PVO rule, with correct AccountabilityProof", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		var p AccountabilityProof
		p.Rule = autonity.PVO
	...
	t.Run("Test validate innocence AccountabilityProof of C1 rule, with wrong msg", func(t *testing.T) {
		var p AccountabilityProof
		p.Rule = autonity.C1
		wrongMsg := newVoteMsg(height, 1, proto.MsgPrevote, proposerKey, noneNilValue, committee)
		p.Message = wrongMsg
	...
	t.Run("Test validate innocence AccountabilityProof of C1 rule, with a wrong preCommit for nil", func(t *testing.T) {
		var p AccountabilityProof
		p.Rule = autonity.C1
	...
	t.Run("Test validate innocence AccountabilityProof of C1 rule, with a wrong evidence", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		var p AccountabilityProof
		p.Rule = autonity.C1
	...
	t.Run("Test validate innocence AccountabilityProof of C1 rule, with redundant msgs in evidence ", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		var p AccountabilityProof
		p.Rule = autonity.C1
	...
	t.Run("Test validate innocence AccountabilityProof of C1 rule, with no quorum votes of evidence ", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		var p AccountabilityProof
		p.Rule = autonity.C1
	...
	t.Run("Test validate innocence AccountabilityProof of C1 rule, with correct evidence ", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		var p AccountabilityProof
		p.Rule = autonity.C1
...
func TestCheckEquivocation(t *testing.T) {
	...
	t.Run("check equivocation with valid AccountabilityProof of equivocation", func(t *testing.T) {
		proposal := newProposalMessage(height, round, -1, keys[committee[0].Address], committee, nil)
		...
		var proofs []*mUtils.Message
		proofs = append(proofs, vote2)
		require.Equal(t, errEquivocation, checkEquivocation(vote1, proofs))
	})

	t.Run("check equivocation with invalid AccountabilityProof of equivocation", func(t *testing.T) {
		proposal := newProposalMessage(height, round, -1, keys[committee[0].Address], committee, nil)
		vote1 := newVoteMsg(height, round, proto.MsgPrevote, keys[committee[0].Address], proposal.Value(), committee)
		var proofs []*mUtils.Message
...
func TestReturnBytes(t *testing.T) {
	...
	rule := autonity.PN
	bytes := resultBytes(msgHash, msgSender, rule)
	require.Equal(t, 128, len(bytes))
}
