...
func liteProposalSignature(h uint64, r int64, vr int64, value common.Hash, signerKey *ecdsa.PrivateKey) []byte {
	...
	sigLiteProposal, err := crypto.Sign(hashData, signerKey)
	if err != nil {
		return nil
	}
	return sigLiteProposal
}
...
func newProposalMessage(h uint64, r int64, vr int64, senderKey *ecdsa.PrivateKey, committee types.Committee, withValue *types.Block) *mUtils.Message {
	...
	proposal := &mUtils.Proposal{
		Round:         r,
		Height:        new(big.Int).SetUint64(h),
		ValidRound:    vr,
		ProposalBlock: block,
		LiteSig:       liteProposalSignature(h, r, vr, block.Hash(), senderKey),
	}
	encodeProposal, err := mUtils.Encode(proposal)
	if err != nil {
		return nil
	}
...
func newVoteMsg(h uint64, r int64, code uint8, senderKey *ecdsa.PrivateKey, value common.Hash, committee types.Committee) *mUtils.Message {
	lastHeader := newBlockHeader(h-1, committee)
...
func TestSameVote(t *testing.T) {
	...
	require.Equal(t, true, sameConsensusMsg(proposal, proposal))
	require.Equal(t, false, sameConsensusMsg(proposal, proposal2))
}

func TestIsProposerMsg(t *testing.T) {
	// test get proposer on height 1 since the parent block is genesis block, it elects proposer by round-robin.
	height := uint64(1)
	lastHeight := height - 1
	round := int64(0)
	committee, keys := generateCommittee()
...
func TestSubmitMisbehaviour(t *testing.T) {
	...
	fd.submitMisbehavior(proposal, proofs[:1], errEquivocation, fd.misbehaviourProofsCh)
	p := <-fd.misbehaviourProofsCh

	require.Equal(t, uint8(autonity.Misbehaviour), p.Type)
...
func TestRunRuleEngine(t *testing.T) {
	...
	t.Run("test run rules with malicious behaviour should be detected", func(t *testing.T) {
		...
		fd := NewFaultDetector(chainMock, fdAddr, nil, core.NewMsgStore(), nil, nil, nil)
		...
		require.Equal(t, uint8(autonity.Misbehaviour), onChainProofs[0].Type)
		require.Equal(t, proposer, onChainProofs[0].Sender)
		require.Equal(t, maliciousProposal.ToLiteProposal().MsgHash(), onChainProofs[0].MsgHash)
	})
}
...
func TestProcessMsg(t *testing.T) {
	...
	t.Run("test process future msg, msg should be buffered", func(t *testing.T) {
		...
		var blockSub event.Subscription
		chainMock.EXPECT().SubscribeChainEvent().AnyTimes().Return(blockSub)
		fd := NewFaultDetector(chainMock, proposer, nil, core.NewMsgStore(), nil, nil, nil)
...
func TestGenerateOnChainProof(t *testing.T) {
	...
	var evidence []*mUtils.Message
	evidence = append(evidence, equivocatedProposal)

	p := AccountabilityProof{
	...
	t.Run("Test onChainProof is generated", func(t *testing.T) {
		...
		require.Equal(t, uint8(autonity.Misbehaviour), onChainProof.Type)
	...
	t.Run("Test abi packing of onChainProof", func(t *testing.T) {
		...
		methodName := "handleAccountabilityEvents"
		require.NoError(t, err)

		_, err = defaultABI.Pack(methodName, []autonity.AccountabilityEvent{*onChainProof})
		require.NoError(t, err)
	})
}
...
func TestRuleEngine(t *testing.T) {
	...
	round := int64(3)
	validRound := int64(1)
	totalPower := new(big.Int).SetUint64(uint64(len(committee)))
	...
	t.Run("getInnocentProofOfPO have quorum preVotes", func(t *testing.T) {
		...
		// PO: node propose an old value with an validRound, innocent AccountabilityProof of it should be:
		...
		var blockSub event.Subscription
		chainMock.EXPECT().SubscribeChainEvent().AnyTimes().Return(blockSub)
		fd := NewFaultDetector(chainMock, proposer, new(event.TypeMux).Subscribe(events.MessageEvent{}), core.NewMsgStore(), nil, nil, nil)
		...
		assert.Equal(t, uint8(autonity.Innocence), proof.Type)
	...
	t.Run("getInnocentProofOfPVN have no corresponding proposal", func(t *testing.T) {
		...
		fd := FaultDetector{blockchain: nil, address: proposer, msgStore: core.NewMsgStore()}
	...
	t.Run("getInnocentProofofPVO have quorum preVotes", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Equal(t, uint8(autonity.Innocence), onChainProof.Type)
	...
	t.Run("Test error to rule mapping", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Equal(t, autonity.AccountableGarbageMessage, rule)
	...
	t.Run("RunRule address the misbehaviour of PN rule", func(t *testing.T) {
		...
		onChainProofs := fd.runRulesOverHeight(height, quorum.Uint64())
	...
	t.Run("RunRule address the misbehaviour of PO rule, the old value proposed is not locked", func(t *testing.T) {
		...
		// has proposed a value for which it is not locked on, thus a AccountabilityProof of
	...
	t.Run("RunRule address the misbehaviour of PVN, node preVote for value V1 while it preCommitted another value at previous round", func(t *testing.T) {
		...
		assert.Equal(t, autonity.Misbehaviour, onChainProofs[0].Type)
		assert.Equal(t, autonity.PVN, onChainProofs[0].Rule)
		assert.Equal(t, 4, len(onChainProofs[0].Evidence))
		assert.Equal(t, preVote.Signature, onChainProofs[0].Message.Signature)
		assert.Equal(t, newProposal.ToLiteProposal().Signature, onChainProofs[0].Evidence[0].Signature)
		assert.Equal(t, preCommit.Signature, onChainProofs[0].Evidence[1].Signature)
	})
	...
	t.Run("RunRule to address misbehaviour of rule PVO12, node last precommited at a value of not v", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		chainMock := NewMockBlockChainContext(ctrl)
		var blockSub event.Subscription
		...
		assert.Equal(t, true, presentPVO12)
	})

	t.Run("RunRule to address misbehaviour of rule PVO12, node did precommited at a value of not v between valid "+
