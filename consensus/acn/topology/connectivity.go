...
func (g *faultTolerantTopology) connectivity() (int, error) {
	...
	for node := 0; node < g.nodeCount; node++ {
		for peer := node + 1; peer < g.nodeCount; peer++ {
			if !g.isAdjacent(node, peer) {
				pathCount, err := g.maxVertexIndependentPathCount(node, peer)
				if err != nil {
					return 0, err
				}
				res = min(res, pathCount)
			}
		}
	}
	return res, nil
...
func (g *faultTolerantTopology) maxVertexIndependentPathCount(u, v int) (int, error) {
	...
	for node := 0; node < g.nodeCount; node++ {
		...
		edges, err := g.RequestSubset(node)
		if err != nil {
			return 0, err
		}
	...
	return graph.DinitzMaxFlow(u+g.nodeCount, v), nil
}
