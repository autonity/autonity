...
func newTestBackend(t *testing.T, n int, gspec *core.Genesis, engine consensus.Engine, generator func(i int, b *core.BlockGen)) *testBackend {
	...
	chain, err := core.NewBlockChain(db, cacheConfig, gspec, engine, vm.Config{}, &txlookupLimit, core.FakeContractBackendProvider(t), log.Root())
	if err != nil {
...
func (b testBackend) Eip1559ParamsByHeight(height uint64) (*types.Eip1559Params, error) {
	return &types.Eip1559Params{
		MinBaseFee:               common.Big0, // Use 0 to match Ethereum standard test behavior
...
func TestEstimateGas(t *testing.T) {
	...
	for i, tc := range testSuite {
		// Skip tests using post-merge opcodes (PUSH0, EOF) not available in Autonity test config
		if i == 9 || i == 10 {
			continue
		}
...
func TestCall(t *testing.T) {
	...
	var (
		...
		genesis  = &core.Genesis{
			Config:     params.TestConfigNoVerkle,
			Difficulty: new(big.Int).SetUint64(131072),
			Alloc: types.GenesisAlloc{
				accounts[0].addr: {Balance: big.NewInt(params.Ether)},
				accounts[1].addr: {Balance: big.NewInt(params.Ether)},
	...
	for _, tc := range testSuite {
		// Skip tests using post-merge opcodes not available in Autonity test config
		if tc.name == "block-override-with-state-override" || tc.name == "blobhash-opcode" {
			continue
		}
...
func TestSimulateV1(t *testing.T) {
	...
	for _, tc := range testSuite {
		t.Run(tc.name, func(t *testing.T) {
			// Skip tests with hardcoded base fee values incompatible with Autonity's InitialBaseFee
			if tc.name == "validation-checks-from-contract" || tc.name == "basefee-validation-mode" {
				t.Skip("hardcoded base fee values incompatible with Autonity")
			}
...
func TestSimulateV1ChainLinkage(t *testing.T) {
	t.Skip("hardcoded blockhash values incompatible with Autonity simulation")
	var (
		acc          = newTestAccount()
		sender       = acc.addr
		...
		signer = types.LatestSigner(params.TestConfigNoVerkle)
	)
...
func TestSignTransaction(t *testing.T) {
	...
	b := newTestBackend(t, 1, genesis, ethash.NewFaker(), func(i int, b *core.BlockGen) {
	})
	...
	expect := `{"type":"0x2","chainId":"0x1","nonce":"0x0","to":"0x703c4b2bd70c169f5717101caee543299fc946c7","gas":"0x5208","gasPrice":null,"maxPriorityFeePerGas":"0x0","maxFeePerGas":"0x77359400","value":"0x1","input":"0x","accessList":[],"v":"0x1","r":"0xa156915e0124cd4d321fa9109843b54aa7bfa9cfde2b408639386e633c45a71b","s":"0x45a1b2f68235c9a7ffe1111145108d58053b6e51a01aee7286c724d4319b81be","yParity":"0x1","hash":"0x4c5a6d7edfce4a1b5a415b949c4def18e8ffb7bbd3450f8afd1e2511aad470ee"}`
...
func TestRPCMarshalBlock(t *testing.T) {
	t.Skip("hardcode values incompatible with autonity")
	t.Parallel()
	var (
...
func TestRPCGetBlockOrHeader(t *testing.T) {
	t.Parallel()
	t.Skip("hardcoded values not compatible with autonity")
	// Initialize test accounts
	var (
...
func setupReceiptBackend(t *testing.T, genBlocks int) (*testBackend, []common.Hash) {
	...
	var (
		...
		signer   = types.LatestSignerForChainID(params.TestConfigNoVerkle.ChainID)
