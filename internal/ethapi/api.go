...
func (api *TxPoolAPI) Content() map[string]map[string]map[string]*RPCTransaction {
	...
	for account, txs := range pending {
		...
		for _, tx := range txs {
			dump[fmt.Sprintf("%d", tx.Nonce())] = NewRPCPendingTransaction(tx, curHeader, api.b.ChainConfig(), api.b)
		}
...
func (context *ChainContext) GetHeaderByNumber(u uint64) *types.Header {
	header, err := context.b.HeaderByNumber(context.ctx, rpc.BlockNumber(u))
	if err != nil {
		return nil
	}
	return header
...
func doCall(ctx context.Context, b Backend, args TransactionArgs, state *state.StateDB, header *types.Header, overrides *override.StateOverride, blockOverrides *override.BlockOverrides, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, error) {
	...
	rules := b.ChainConfig().Rules(blockCtx.BlockNumber, blockCtx.Random != nil)
...
func NewRPCPendingTransaction(tx *types.Transaction, current *types.Header, config *params.ChainConfig, backend Eip1559ParamsByHeightProvider) *RPCTransaction {
	var (
		baseFee     *big.Int
...
func (api *TransactionAPI) GetTransactionByHash(ctx context.Context, hash common.Hash) (*RPCTransaction, error) {
	...
	if !found {
		...
		if tx := api.b.GetPoolTransaction(hash); tx != nil {
			return NewRPCPendingTransaction(tx, api.b.CurrentHeader(), api.b.ChainConfig(), api.b), nil
		}
		if err == nil {
			return nil, nil
		}
...
func (api *TransactionAPI) PendingTransactions() ([]*RPCTransaction, error) {
	...
	for _, tx := range pending {
		...
		if _, exists := accounts[from]; exists {
			transactions = append(transactions, NewRPCPendingTransaction(tx, curHeader, api.b.ChainConfig(), api.b))
		}
	}
