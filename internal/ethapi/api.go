...
func NewRPCPendingTransaction(tx *types.Transaction, current *types.Header, config *params.ChainConfig) *RPCTransaction {
	...
	if current != nil {
		// todo(youssef): the following is not ideal but to be able to recover
		// the minimum base fee for any previous block we need access to historical
		// state data which is not possible. A better solution would be to include the
		// minimum base fee information within the epoch block header.
		// Hence for the time being, the transaction gasprice returned data will be wrong.
