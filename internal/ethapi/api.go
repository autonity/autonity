...
func (s *PublicTxPoolAPI) Content() map[string]map[string]map[string]*RPCTransaction {
	...
	for account, txs := range pending {
		...
		for _, tx := range txs {
			dump[fmt.Sprintf("%d", tx.Nonce())] = newRPCPendingTransaction(tx, curHeader, s.b.ChainConfig(), s.b)
		}
...
//
// https://github.com/autonity/autonity/wiki/Management-APIs#personal_sign
...
//
// https://github.com/autonity/autonity/wiki/Management-APIs#personal_ecRecover
...
func newRPCPendingTransaction(tx *types.Transaction, current *types.Header, config *params.ChainConfig, backend Backend) *RPCTransaction {
	var baseFee *big.Int
	blockNumber := uint64(0)
	if current != nil {
		eip1559Params, err := backend.Eip1559ParamsByHeight(current.Number.Uint64() + 1)
		if err == nil {
			baseFee = misc.CalcBaseFee(config, current, eip1559Params)
		}
...
func (s *PublicTransactionPoolAPI) GetTransactionByHash(ctx context.Context, hash common.Hash) (*RPCTransaction, error) {
	...
	if tx := s.b.GetPoolTransaction(hash); tx != nil {
		return newRPCPendingTransaction(tx, s.b.CurrentHeader(), s.b.ChainConfig(), s.b), nil
	}
...
func (s *PublicTransactionPoolAPI) GetTransactionReceipt(ctx context.Context, hash common.Hash) (map[string]interface{}, error) {
	// TODO: special case for an autonity contract receipt
	tx, blockHash, blockNumber, index, err := s.b.GetTransaction(ctx, hash)
	if err != nil {
		return nil, nil
	}
...
func (s *PublicTransactionPoolAPI) PendingTransactions() ([]*RPCTransaction, error) {
	...
	for _, tx := range pending {
		...
		if _, exists := accounts[from]; exists {
			transactions = append(transactions, newRPCPendingTransaction(tx, curHeader, s.b.ChainConfig(), s.b))
		}
	}
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f auton) exceeds the configured cap (%.2f auton)", feeFloat, cap)
	}
	return nil
}
