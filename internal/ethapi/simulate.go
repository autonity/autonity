...
func (m *simChainHeadReader) EpochByHeight(height uint64) (*types.EpochInfo, error) {
	//TODO implement me
	panic("implement me")
}

func (m *simChainHeadReader) GetBlock(hash common.Hash, number uint64) *types.Block {
	//TODO implement me
	panic("implement me")
}
...
func (sim *simulator) processBlock(ctx context.Context, block *simBlock, header, parent *types.Header, headers []*types.Header, timeout time.Duration) (*types.Block, []simCallResult, error) {
	...
	if sim.chainConfig.IsLondon(header.Number) {
		...
		if header.BaseFee == nil {
			if sim.validate {
				eip1559Params, err := sim.b.Eip1559ParamsByHeight(header.Number.Uint64())
				if err != nil {
					return nil, nil, fmt.Errorf("failed to get EIP-1559 params: %w", err)
				}
				header.BaseFee = misc.CalcBaseFee(sim.chainConfig, parent, eip1559Params)
			} else {
