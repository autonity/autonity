...
If you wish to attach to one of the nodes from your host machine first build
autonity with the following command.
...
Then use the built binary to attach to a console. Here is an example of
attaching a console to `autonity-node-1` from the host machine, note that the
host port for a container that corresponds to 8545 can be found from the output
of `docker-compose ps` in this case the port is 8541.

```bash
$ ../build/bin/autonity attach http://localhost:8541
Welcome to the Autonity JavaScript console!
...
First of all we need to connect to one of the nodes: `../build/bin/autonity attach http://localhost:8541`
