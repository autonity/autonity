  ...
  testcases:
    ...
    - name: "TC-3: With 2 node crashed, N = 6, Q = ceil(2xN/3) = 4, not on-holding."
      condition:
        ...
        scenario:
          - delay: 20 # stop the node after test running for 20 seconds.
            action: "stop"
    ...
    - name: "TC-4: With 2 node crashed, 1 node back, after the back, consensus should not on-holding."
      condition:
        crashNodes: [0, 1] # contain the node domain name we want to crash.
        scenario:
          - delay: 20 # stop the node after test running for 65 seconds.
            action: "stop"
            target: [0, 1] # select candidates from crashNodes.
          - delay: 40 # start the node after test running for 60 seconds.
            action: "start"
      ...
      condition:
        ...
        scenario:
          - delay: 20 # stop the node after test running for 20 seconds.
            action: "stop"
            target: [0, 1] # select candidates from crashNodes.
          - delay: 60 # start the node after test running for 60 seconds.
            action: "start"
            target: [0, 1] # select candidates from crashNodes.
      input:
        senderNode: 2  # node coin_base address will be used as default sender address.
    ...
    - name: "TC-6: With 3 node crashed, 3 of 6 crashed, system on-hold."
      condition:
        ...
        scenario:
          - delay: 20 # stop the node after test running for 5 seconds.
            action: "stop"
    ...
    - name: "TC-7: With 3 node crashed, 1 node recover, consensus engine should not on-hold."
      condition:
        crashNodes: [0, 1, 4] # contain the node domain name we want to crash.
        scenario:
          - delay: 10 # stop the node after test running for seconds.
            action: "stop"
            target: [0, 1, 4] # select candidates from crashNodes.
    ...
    - name: "TC-8: With 3 node crashed, 2 node recover, consensus engine should not on-hold."
      condition:
        crashNodes: [0, 1, 4] # contain the node domain name we want to crash.
        scenario:
          - delay: 20 # stop the node after test running for 2 seconds.
            action: "stop"
            target: [0, 1, 4] # select candidates from crashNodes.
          - delay: 60 # start the node after test running for 30 seconds.
            action: "start"
            target: [0, 1] # select candidates from crashNodes.
      input:
        senderNode: 2  # node coin_base address will be used as default sender address.
    ...
    ###################################bootstrap tests########################
    - name: "TC-10: Bootstrap from scratch with 1 node crashed and recover later during the bootstrap."
      re-deploy: true
      condition:
    ...
    - name: "TC-11: Bootstrap from scratch, 2 by 2 on 5 seconds interval"
      re-deploy: true
      condition:
        crashNodes: [] # contain the node domain name we want to crash.
        scenario:
    ...
    ###################################partitioning tests#####################
    - name: "TC-12: Network partition. 6 break into 2 slices with 3 node each slice, engine should on-hold."
      condition:
        crashNodes: [] # contain the node domain name we want to crash.
        scenario:
    ...
    - name: "TC-13: Network partition. 6 break into 3 and 3, then the two slices recovers, engine recovers too."
      condition:
        crashNodes: [] # contain the node domain name we want to crash.
        scenario:
    ...
    - name: "TC-14: Network partition. 6 break into 4 and 2, main party(4 validator) should not on-hold."
      condition:
        crashNodes: []
        scenario:
          - delay: 30 # on the 5th second, disconnect peers.
            action: "disconnect" # it apply p2p disconnection by managing ip-tables in linux kernel.
    ...
    #################special topologies#######################################
    - name: "TC-15: 6 nodes in Ring network topology, should not on-hold."
      condition:
        crashNodes: [] # contain the nodes which leave the main party.
        scenario:
          - delay: 25 # on the 25th second, disconnect peers.
            action: "disconnect" # it apply p2p disconnection by managing ip-tables in linux kernel.
    ...
    - name: "TC-16: 6 nodes in bus line network topology, should not on-hold."
      condition:
        crashNodes: [] # contain the nodes which leave the main party.
        scenario:
          - delay: 2 # on the 5th second, disconnect peers.
    ...
    - name: "TC-17: 6 nodes in star network topology, should not on-hold."
      condition:
        crashNodes: [] # contain the nodes which leave the main party.
        scenario:
          - delay: 2 # on the 5th second, disconnect peers.
    ...
    - name: "TC-18: network traffic control with package delay, loss, duplication, reordering, corruption."
      condition:
        crashNodes: [] # contain the node domain name we want to crash.
        scenario:
    ...
    ########################bootstrap test#######################################
    - name: "TC-19: Bootstrap from scratch, with all node start in the same time."
      re-deploy: true
      condition:
        crashNodes: [] # contain the node domain name we want to crash.
        scenario:
    ...
    - name: "TC-26: Network partition. 6 break into 3 and 3, then the two slices connected by bridge, engine recovers too."
      condition:
        crashNodes: [] # contain the node domain name we want to crash.
        scenario:
          - delay: 20 # on the 10th second, disconnect peers.
            action: "disconnect" # it apply p2p disconnection by managing ip-tables in linux kernel.
            peers: [[4, 0], [4, 1], [4, 2], [5, 0], [5, 1], [5, 2], [3, 0], [3, 1], [3, 2]]
          - delay: 40 # on the 90th second, connect peers.
            action: "connect" # it apply p2p disconnection by managing ip-tables in linux kernel.
            peers: [[4, 0]]
      input:
        senderNode: 2  # node coin_base address will be used as default sender address.
    ...
    - name: "TC-27: Network partition. 6 break to 3 full mesh plus 1 node, and 2 nodes of bus line, recover with 3 nodes of full mesh plus a 3 nodes of bus line."
      condition:
        ...
        scenario:
          ...
          - delay: 40 # on the 25th second, connect peers. 3 + 1, start producing block.
            action: "connect" # it apply p2p disconnection by managing ip-tables in linux kernel.
            peers: [[4, 0]]
          # break the alone node from 3 full mesh,
          - delay: 60 # on the 65th second, disconnect peers, on-hold.
            action: "disconnect" # turn to 3 + 1 + 2
            peers: [[4, 0]]
          # connect the left node in bus line 65.198-----79.95-----85.250
          - delay: 80 # on the 125th second, turn to 6 with 3 node in full mesh + 3 node in bus line.
