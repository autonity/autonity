...
func activateValidator(t *testing.T, network e2e.Network, autonity *autonity.Autonity, validatorID common.Address, treasuryKey *ecdsa.PrivateKey) {
	...
	require.NotEqual(t, uint8(0), val.State)
	if val.State == uint8(2) {
		require.LessOrEqual(t, val.JailReleaseBlock.Uint64(), currentHeight)
	}
	require.NotEqual(t, uint8(3), val.State)
	...
	require.NoError(t, err)
	require.Equal(t, uint8(0), validator(t, autonity, validatorID).State)
}
...
func TestOmissionFaultyNode(t *testing.T) {
	...
	require.Equal(t, uint8(2), faultyValidator.State)
	...
	require.Equal(t, uint8(2), validator(t, autonityContract, faultyNode).State) // should still be jailed
	...
	require.Equal(t, uint8(2), validator(t, autonityContract, network[2].Address).State) // should still be jailed
...
// this can be controlled by tweaking the lookback window. A reasonable value for look-back windowdepends on many factors:
...
func (c *effortTrackerProposer) SendProposal(ctx context.Context, proposal *types.Block) {
	...
	committee, _, _, _, err := backend.BlockChain().LatestEpoch()
	if err != nil {
