...
func (s *AccusationRulePOBroadcaster) SignAndBroadcast(ctx context.Context, msg *message.Message) {
	...
	invalidProposal := e2e.NewProposeMsg(s.Address(), p.ProposalBlock, msg.H(), nPR, vR, s.Backend().Sign, msg.Power)
	signature, err := s.SignMessage(invalidProposal)
	if err != nil {
		s.Logger().Warn("Cannot simulate accusation for rule PO", err)
	}
	invalidProposal.Signature = signature
	e2e.DefaultSignAndBroadcast(ctx, s.Core, msg)
	s.Logger().Info("Accusation of PO rule is simulated")
	_ = s.Backend().Broadcast(ctx, s.CommitteeSet().Committee(), invalidProposal)
}
...
func (s *AccusationRulePVNBroadcaster) SignAndBroadcast(ctx context.Context, msg *message.Message) {
	...
	_ = msg.DecodePayload()
	preVote := e2e.NewVoteMsg(consensus.MsgPrevote, msg.H(), msg.R()+1, e2e.NonNilValue, s.Core, msg.Power)
	signature, err := s.SignMessage(preVote)
	if err != nil {
	...
	e2e.DefaultSignAndBroadcast(ctx, s.Core, msg)
	_ = s.Backend().Broadcast(ctx, s.CommitteeSet().Committee(), preVote)
}
...
func (s *AccusationRulePVOBroadcaster) SignAndBroadcast(ctx context.Context, msg *message.Message) {
	...
	msgProposal := e2e.NewProposeMsg(s.Address(), p.ProposalBlock, msg.H(), nPR, validRound, s.Core.Backend().Sign, msg.Power)
	signature, err := s.SignMessage(msgProposal)
	if err != nil {
		s.Logger().Warn("Cannot simulate accusation for rule PVO", err)
	}
	msgProposal.Signature = signature
	// simulate a preVote at round nPR, for value v, this preVote for new value break PVO1.
	msgPVO1 := e2e.NewVoteMsg(consensus.MsgPrevote, msg.H(), nPR, p.V(), s.Core, msg.Power)
	signature, err = s.SignMessage(msgPVO1)
	if err != nil {
		s.Logger().Warn("Cannot simulate accusation for rule PVO", err)
	}
	msgPVO1.Signature = signature
	s.Logger().Info("Accusation of PVO rule is simulated.")
	e2e.DefaultSignAndBroadcast(ctx, s.Core, msg)
	_ = s.Backend().Broadcast(ctx, s.CommitteeSet().Committee(), msgProposal)
	_ = s.Backend().Broadcast(ctx, s.CommitteeSet().Committee(), msgPVO1)
}
...
func (s *AccusationRuleC1Broadcaster) SignAndBroadcast(ctx context.Context, msg *message.Message) {
	...
	if s.IsProposer() {
		preCommit := e2e.NewVoteMsg(consensus.MsgPrecommit, msg.H(), nPR, p.V(), s.Core, msg.Power)
		signature, err := s.SignMessage(preCommit)
		if err != nil {
			s.Logger().Warn("Cannot simulate accusation for rule C1", err)
		}
		preCommit.Signature = signature
		s.Logger().Info("Accusation of C1 rule is simulated.")
		_ = s.Backend().Broadcast(ctx, s.CommitteeSet().Committee(), preCommit)
	}
	e2e.DefaultSignAndBroadcast(ctx, s.Core, msg)
}
