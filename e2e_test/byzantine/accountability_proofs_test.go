...
func (s *PN) SignAndBroadcast(ctx context.Context, msg *message.Message) {
	...
	msgEvidence := e2e.NewVoteMsg(consensus.MsgPrecommit, msg.H(), nPR-1, e2e.NonNilValue, s.Core, msg.Power)
	...
	msgPN := e2e.NewProposeMsg(s.Core.Address(), msg.ConsensusMsg.(*message.Proposal).ProposalBlock, msg.H(), nPR, -1, s.Core.Backend().Sign, msg.Power)
	printMessage(msgPN)
...
func (s *PO) SignAndBroadcast(ctx context.Context, proposal *message.Message) {
	...
	msgEvidence := e2e.NewVoteMsg(consensus.MsgPrecommit, proposal.H(), vR, e2e.NonNilValue, s.Core, proposal.Power)
	printMessage(msgEvidence)
	// simulate a proposal that node propose for an old value which it is not the one it locked.
	msgPO := e2e.NewProposeMsg(s.Address(), proposal.ConsensusMsg.(*message.Proposal).ProposalBlock, proposal.H(), nPR, vR, s.Core.Backend().Sign, proposal.Power)
	printMessage(msgPO)
...
func (s *PVN) SignAndBroadcast(ctx context.Context, proposal *message.Message) {
	...
	newProposal := e2e.NewProposeMsg(s.Address(), newBlock, proposal.H(), nPR, -1, s.Core.Backend().Sign, proposal.Power)
	...
	precommit := e2e.NewVoteMsg(consensus.MsgPrecommit, proposal.H(), proposal.R(), proposal.ConsensusMsg.V(), s.Core, proposal.Power)
	// simulate nil precommits until nPr to get contiguous precommits
	for i := proposal.R() + 1; i < nPR; i++ {
		nilPrecommit := e2e.NewVoteMsg(consensus.MsgPrecommit, proposal.H(), i, core.NilValue, s.Core, proposal.Power)
	...
	evidence := e2e.NewVoteMsg(consensus.MsgPrevote, proposal.H(), nPR, newProposal.Value(), s.Core, proposal.Power)
...
func (s *PVO1) SignAndBroadcast(ctx context.Context, msg *message.Message) {
	...
	proposal := e2e.NewProposeMsg(s.Address(), msg.ConsensusMsg.(*message.Proposal).ProposalBlock, msg.H(), round, validRound, s.Backend().Sign, msg.Power)
	e2e.DefaultSignAndBroadcast(ctx, s.Core, proposal)
	...
	for r := validRound; r < round; r++ {
		...
		precommit := e2e.NewVoteMsg(consensus.MsgPrecommit, proposal.H(), r, val, s.Core, proposal.Power)
		e2e.DefaultSignAndBroadcast(ctx, s.Core, precommit)
	}
	evidence := e2e.NewVoteMsg(consensus.MsgPrevote, proposal.H(), round, proposal.Value(), s.Core, proposal.Power)
	e2e.DefaultSignAndBroadcast(ctx, s.Core, evidence)
...
func (s *InvalidProposal) SignAndBroadcast(ctx context.Context, msg *message.Message) {
	...
	msgP := e2e.NewProposeMsg(s.Address(), block, msg.H(), nextPR, msg.ConsensusMsg.(*message.Proposal).ValidRound, s.Backend().Sign, msg.Power)
	signature, err := s.SignMessage(msgP)
	if err != nil {
	...
	e2e.DefaultSignAndBroadcast(ctx, s.Core, msg)
	_ = s.Backend().Broadcast(ctx, s.CommitteeSet().Committee(), msgP)
}
...
func (s *InvalidProposer) SignAndBroadcast(ctx context.Context, msg *message.Message) {
	...
	header := &types.Header{Number: new(big.Int).SetUint64(msg.H())}
	block := types.NewBlockWithHeader(header)
	msgP := e2e.NewProposeMsg(s.Address(), block, msg.H(), msg.R(), -1, s.Backend().Sign, msg.Power)
	signature, err := s.SignMessage(msgP)
	if err != nil {
...
func (s *Equivocation) SignAndBroadcast(ctx context.Context, msg *message.Message) {
	...
	if msg.Code == consensus.MsgPrevote && s.IsProposer() {
		msgEq := e2e.NewVoteMsg(consensus.MsgPrevote, msg.H(), msg.R(), e2e.NonNilValue, s.Core, msg.Power)
		signature, err := s.SignMessage(msgEq)
		if err != nil {
			s.Logger().Warn("Cannot simulate Misbehaviour of equivocation rule", err)
		}
		msgEq.Signature = signature
		s.Logger().Info("Misbehaviour of equivocation rule is simulated.")
		_ = s.Backend().Broadcast(ctx, s.CommitteeSet().Committee(), msgEq)
	}
}
