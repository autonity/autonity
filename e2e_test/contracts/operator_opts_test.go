...
func TestOperatorOpts(t *testing.T) {
	...
	err = client.AwaitSetOperator(initialOptKey, newOperator, timeout)
	require.NoError(t, err)
	...
	err = client.AwaitSetCommitteeSize(newOptKey, newSize, timeout)
	require.NoError(t, err)
	...
	err = client.AwaitSetUnbondingPeriod(newOptKey, newBondingPeriod, timeout)
	require.NoError(t, err)
	...
	err = client.AwaitSetEpochPeriod(newOptKey, newEpochPeriod, timeout)
	require.NoError(t, err)
	...
	err = client.AwaitSetTreasuryAccount(newOptKey, newTreasuryAddr, timeout)
	require.NoError(t, err)
	...
	err = client.AwaitSetTreasuryFee(newOptKey, newTreasuryFee, timeout)
	require.NoError(t, err)
...
func TestUpgradeAC(t *testing.T) {
	...
	client := network[0]
	optKey := client.Key
	err = client.AwaitUpgradeAC(optKey, bytecode[0:len(bytecode)/2], "", timeout)
	require.NoError(t, err)

	err = client.AwaitUpgradeAC(optKey, bytecode[len(bytecode)/2:], "", timeout)
	require.NoError(t, err)

	res, err := json.Marshal(&abi)
	require.NoError(t, err)
	err = client.AwaitUpgradeAC(optKey, nil, string(res), timeout)
	require.NoError(t, err)

	err = client.AwaitCompleteUpgradeAC(optKey, timeout)
	require.NoError(t, err)
