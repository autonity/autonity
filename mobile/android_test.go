...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Autonity.LightScryptN, Autonity.LightScryptP);
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "autonity-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ethereum/autonitytest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Autonity and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/clearmatics/autonity/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "autonity.aar"), "autonity.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ethereum", "autonitytest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.ethereum.autonitytest"
